...
- **仍未完成：**C-4P9 整个 gate 未关闭。S2 的实现与 S3/S4/S5/S6 evidence 不表示 generic JSONL migration、跨文件 transaction、ledger authority 或 archive save-order 变更、repair、rotation、IPC/UI 已实施；S5 只补 capability symmetry 的这 20 个定向 tests-only cases，不是完整 capability matrix；S6 只补 ledger-own failure residual 的定向 tests-only cases，不是完整 residual matrix 或生产功能。并发（超出既有 per-path linearization）、trace residual、generic JSONL、rotation、事务、ledger authority/save order 改造、repair、IPC/UI 及其它 residual matrix 仍保留在 design gate 中，继续等待后续批准。
...
- **S1/S2/S3/S4/S5 必须分开计数和命名：**S1 是 41 项 unit + 14 项 integration 的历史有限证据；S2 只覆盖单一 `after_outcome_publish` crash window；S3 只覆盖 marker final rename `EIO` failure/restart/reconcile，不是 generic post-manifest；S4 只覆盖单一已有 `after_settlement_marker` interruption 的 settled recovery，实际定向结果为 1 file / 29 tests passed；S5 只覆盖单一已有 `before_catalog_reconcile` interruption 的 settled recovery，实际定向结果为 1 file / 30 tests passed。S3 不是泛化 `after_manifest_publish`、完整 manifest failure matrix、生产功能或完整 C-4P6 closure；S4/S5 同样不是完整 C-4P6、完整 catalog/manifest/crash matrix、transaction、rollback、delete、migration、API、operations validation 或 Windows native fsync/power-loss closure。
- **仍未完成：**完整 C-4P6 因 manifest capability-policy、manifest `open` / `write` / `fsync` / `close` 完整矩阵、其它 crash/failure（含 `after_stage_flush` / `after_record_publish` 等尚未单独计为 evidence 的窗口）、跨文件 transaction、rollback、delete、migration、API、operations validation、Windows native fsync/power-loss closure 及完整 close-out 尚未完成，必须继续保留在本待办；S5 仅关闭 `before_catalog_reconcile` 这一条定向 evidence。不要删除 design/todo。
