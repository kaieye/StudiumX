...
上述 evidence slice 都没有生产语义改动。**P9-S3 保留 partial-write 与 archive-level failure/retry 的实际历史证据；P9-S4 仅补齐首个 audit write `EIO`、0 audit bytes 的 archive-save short-circuit/retry evidence；P9-S5 仅补齐上述 20 个 directory capability symmetry cases；P9-S6/S7/S8/S9/S10 分别仅补齐 ledger-own failure、concurrent identical same-save、divergent-trace conflict、concurrent same-ID body conflict 与 Markdown durable publish short-circuit 的定向 evidence；C-4P9 仍未关闭。**
...
| 测试类别 | 最低验证 | P9-S3..S10 evidence 状态 |
|---|---|---|
