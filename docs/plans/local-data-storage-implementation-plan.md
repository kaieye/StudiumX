...
### 1.6 `database` 实施审计（2026-07-18；C-4P0 `5c0dd96`、C-4P1 `34c48f4`、C-4P2A `b8eb3ab`、C-4P2B `70afe1d`、C-4P3 `99bf6fe`、C-4P4 `f8ad99c`、C-4P5 `278f141`、C-4P7 `0d55fd8`；C-5G 功能代码 `1bbdf7c`、测试补充 `e63e051`）

原始顺序已完成最小切片：C-2A `d23b272`、C-2B `549f4f8`、C-1 `d9de382`、C-3/C-4 `ca73537`、C-2C `07dfbfb`、C-7 `a302814`、C-5 `55442ad`、C-6 `26eca18`；后续 C-5B Memory CRUD trace correlation 为 `7a1ca7e`，C-5C learning-session trace 为 `e849d51`，C-5D `saveAgentConversation()` lifecycle trace 为 `dee70d6`，C-5E conversation audit JSONL trace 为 `d6a94a1`，C-5F forked conversation trace 为 `426eb6e`，C-5G workspace activation lifecycle 功能代码为 `1bbdf7c`，测试补充为 `e63e051`。`1bbdf7c` 是本次记录的**功能代码提交**，`e63e051` 是其后的**测试补充提交**；两者均不是后续文档提交后的当前 HEAD。C-4 的后续已迁移 consumer 是 review progress C-4P0 `5c0dd96`、conversation archive canonical JSON/Markdown C-4P1 `34c48f4`、workspace `MISSION.md` update C-4P2A `b8eb3ab` 与 workspace `assets/lesson.css` C-4P2B `70afe1d`，以及 cross-workspace change-history `history.json` C-4P3 `99bf6fe` 与 workspace session-open-state sidecar C-4P4 `f8ad99c`，以及 TeachingWorkspaceDocuments allowlisted workspace Markdown C-4P5 `278f141`，以及 MusicCookieStore 私有登录 cookie 状态 C-4P7 `0d55fd8`；这仍不是“所有 writer 已迁移”的声明。C-4P6 仅登记 learning-outcome durable settlement 的 design gate（`docs/plans/local-data-learning-outcome-durable-settlement-design.md`），不是代码/测试实施、迁移完成或 settlement durable closure；C-4P9 仅登记 session-audit durable append 的 design gate（`docs/plans/local-data-session-audit-durable-append-design.md`），未实施、未批准，且没有可计入 C-4P9 的代码或测试结果。各节仍定义边界；未实现的 FTS、物理 retention/删除、自动摘要调度、C-5 的其它 lifecycle producers 与其它 user actions（需设计），以及 C-6 controlled legacy 搬迁保持未实施。
...
| C-3/C-4 | `src/main/persistence/durable-file.ts:81-312`；C-4P0 review：`src/main/teaching-workspace/review.ts:54-73`；C-4P1 archive：`src/main/agent-conversation-archive.ts:57-125`；C-4P2A MISSION：`src/main/teaching-workspace.ts:622-649`；C-4P2B lesson.css：`src/main/teaching-workspace.ts:659-684`；C-4P3 change-history：`src/main/teaching-workspace-change-history.ts:13-92`、`src/main/teaching-workspace-change-audit.ts:57-98`；C-4P4 session-open-state：`src/main/agent-conversation-session-tree.ts:31-32, 650-730, 1108-1146, 1334-1348`；C-4P5 Markdown：`src/main/teaching-workspace-documents.ts:69-96, 221-273`、`src/main/teaching-workspace.ts:1770-1785`；C-4P7 music cookies：`src/main/music/music-cookie-store.ts:50-162` | `tests/unit/durable-file.unit.test.ts:99-246`；`tests/unit/teaching-durable-state.unit.test.ts:37-215`；`tests/unit/teaching-workspace-review-durable.unit.test.ts`；`tests/unit/agent-conversation-archive-durable.unit.test.ts`；`tests/unit/teaching-workspace-mission-durable.unit.test.ts`；`tests/unit/teaching-workspace-lesson-style-durable.unit.test.ts`；`tests/unit/teaching-workspace-change-history-durable.unit.test.ts`；`tests/unit/music-cookie-store-durable.unit.test.ts`；`tests/integration/teaching-workspace-change-audit.integration.test.ts` |
...
`ca73537` 已把 `learning-session-ledger.ts` 的 temp write → file fsync → rename → directory fsync 语义抽为窄共享原语，并按 consumer 接入关键状态。`5c0dd96`（C-4P0）将 review canonical `.studiumx/progress.json` 迁入该路径；`34c48f4`（C-4P1）将 conversation archive 的 canonical JSON/Markdown publish 迁入同一路径；`b8eb3ab`（C-4P2A）仅将 workspace `updateMission()` 的 canonical `MISSION.md` update 迁入同一路径；`70afe1d`（C-4P2B）仅将 `applyLessonStyle()` 的 canonical `assets/lesson.css` publish 迁入同一路径；`99bf6fe`（C-4P3）仅将 cross-workspace `<appDataRoot>/learning-changes/history.json` publish 迁入同一路径；`f8ad99c`（C-4P4）仅将 workspace `.agent-sessions/session-open-state.v1.json` sidecar publish 迁入同一路径；`278f141`（C-4P5）仅将 TeachingWorkspaceDocuments allowlisted workspace Markdown canonical publish 迁入同一路径；`0d55fd8`（C-4P7）仅将 MusicCookieStore 私有登录 cookie 状态 `<userData>/music-cookies.json` 迁入同一路径并显式 `0600`。C-4 只记录共享原语与已迁移 consumer，**不表示所有 writer 已迁移**；高频日志仍不强行改为逐条 fsync；C-4P1 不改变 conversation audit 的既有普通 append 策略，也不改变 learning-work ledger 既有 durable JSONL append 的 file + directory fsync 策略。C-4P6 只记录 `docs/plans/local-data-learning-outcome-durable-settlement-design.md` 的 design gate：future approved follow-up 必须先落实 record/`outcome.json`/manifest/marker 的 authority、有序 publish、post-link parent-directory failure propagation、controlled repair 与 owner/lock 决策；本次没有迁移 consumer、运行测试或完成 settlement durable closure。 C-4P8 只记录 `docs/plans/local-data-workspace-tool-durable-publish-design.md` 的审查否决 design gate：`write_workspace_file` 不可直接采用 `replaceDurably()`；descriptor-bound containment、原子 no-clobber create、overwrite 并发边界、safe error/retry 与 I/O seam 均待分片批准；此 design gate 不记录任何 C-4P8 已实施或已通过的测试结果，也不构成 durable publish completion。 C-4P9 只记录 `docs/plans/local-data-session-audit-durable-append-design.md` 的 design gate：固定 per-conversation audit JSONL 必须保持单文件 non-rotating，C-4P1 的 ordinary append 不可直接迁移；per-path queue、row/torn-tail conflict、post-directory-failure retry、archive/ledger recovery、non-rotation helper 与 I/O seam 均待批准；此 design gate 不记录任何 C-4P9 已实施或已通过的测试结果，也不构成 durable append completion。
...
| workspace MISSION（C-4P2A `b8eb3ab`） | `src/main/teaching-workspace.ts:622-649` | 仅 `updateMission()` 的 canonical `MISSION.md` 用 shared replace publish，保持 render/path、输入清理、既有 root/scope checks 与 `0666 & umask` create mode；不改变 C-5 语义。 |
| workspace lesson style（C-4P2B `70afe1d`） | `src/main/teaching-workspace.ts:659-684` | 仅 `applyLessonStyle()` 的 canonical `assets/lesson.css` 用 shared replace publish，保持 CSS path/content、style validation、workspace/scope checks 与 `0666 & umask` create mode；不改变 renderer 的独立 global settings write；lesson-style correlation/settings transaction/retry/actionId/receipt 属于独立、未批准的 future style design，C-5H 是 mission-first 且不覆盖 style。 |
| cross-workspace change-history（C-4P3 `99bf6fe`） | `src/main/teaching-workspace-change-history.ts:13-92`、`src/main/teaching-workspace-change-audit.ts:57-98` | `<appDataRoot>/learning-changes/history.json` 用 shared replace publish，保持 JSON 格式/path、tolerant read、每 workspace 20 条、`writeQueue` 串行化及 `0666 & umask` create mode；history durable append 成功后才 release retention-evicted checkpoint。 |
| workspace session-open-state（C-4P4 `f8ad99c`） | `src/main/agent-conversation-session-tree.ts:31-32, 650-730, 1108-1146, 1334-1348` | `<workspaceRoot>/.agent-sessions/session-open-state.v1.json` 用 shared replace publish；保持 stable JSON bytes/integrity、64 KiB、256 entries、root-exclusive queue、root containment + symlink preflight 及 `0600` mode。 |
| workspace Markdown（C-4P5 `278f141`） | `src/main/teaching-workspace-documents.ts:69-96, 221-273`、`src/main/teaching-workspace.ts:1770-1785` | `TeachingWorkspaceDocuments.saveMarkdown()` 的 allowlisted workspace Markdown 用 shared replace publish；保持 allowed paths、normalization/traversal 拒绝、parent containment + symlink preflight、final symlink 以 rename 替换而不跟随、metadata readback 和 `0666 & umask` mode；document durable success 后才 touch/save registry。 |
| music cookies（C-4P7 `0d55fd8`） | `src/main/music/music-cookie-store.ts:50-162` | `<userData>/music-cookies.json` 用 shared replace publish，显式 `0600`；保持 schema/default path/normalization/tolerant initial read/API、single-flight initial load 与 mutation queue。`set`/`clear` 仅在 durable success 后确认内存；failure 后下次 mutation 必须先 refresh canonical，refresh read failure 不得基于旧内存 publish。 |
| learning-outcome settlement（C-4P6 design gate，未实施） | `docs/plans/local-data-learning-outcome-durable-settlement-design.md`；只读审计 `src/main/learning-outcome-committer.ts` | 仅定义 future approved follow-up 的 authority、stage → record → outcome → manifest → marker → catalog read 有序协议、failure closure、record-first controlled repair 与 owner/lock 前置决策；不是 consumer migration、测试证据或多文件事务。 |
| 新模块与测试 | `src/main/persistence/durable-file.ts`、`tests/unit/durable-file.unit.test.ts`、`tests/unit/teaching-workspace-review-durable.unit.test.ts`、`tests/unit/agent-conversation-archive-durable.unit.test.ts`、`tests/unit/teaching-workspace-mission-durable.unit.test.ts`、`tests/unit/teaching-workspace-lesson-style-durable.unit.test.ts`；`tests/unit/teaching-workspace-change-history-durable.unit.test.ts`；`tests/unit/agent-conversation-session-tree-durable.unit.test.ts`；`tests/unit/teaching-workspace-documents-durable.unit.test.ts`；`tests/integration/teaching-workspace-change-audit.integration.test.ts`；`tests/integration/teaching-workspace-documents.integration.test.ts` | 以 injected durable operations 覆盖失败、rename、directory sync、review publish、archive 的 JSON → Markdown durable publish order 与 audit/ledger 成功、short-circuit 边界，以及 MISSION 与 lesson.css publish 的 mode、lifecycle/registry short-circuit 边界；history 的 durable order、失败清理、mode/tolerant read/20-entry queue 与 failure 不 release retention checkpoint；session-open-state 的 directory sync/close、old-sidecar/temp cleanup、`0600`、root-exclusive queue/containment preflight 与 branch-open fail-closed 边界；workspace Markdown 的 allowlisted paths、containment/symlink preflight、final symlink 以 rename 替换而不跟随、metadata readback、`0666 & umask`、registry short-circuit 与 directory capability/fatal-error 边界；music cookie store 的 `0600`、single-flight/mutation queue、success 后内存确认、failure 后 canonical refresh、allowlist 与无 path/cookie warning 边界。 |
...
5. **workspace MISSION C-4P2A（`b8eb3ab`）：**仅 `updateMission()` 的 canonical `MISSION.md` update 用 shared replace publish，保持 render/path、输入清理、既有 root/scope checks 与 legacy `0666 & umask` create mode。只有 temp write → file fsync → close → rename → directory fsync/close 成功后，才 append `mission_updated` lifecycle、touch/save registry 并返回成功。write/file-sync/file-close/rename failure 保留旧 MISSION 且清理 temp；rename 后 directory fsync/close failure 不返回成功、不 append lifecycle 或 touch/save registry，完整新 MISSION 可保留为 complete-but-unacknowledged，绝不自动 rollback。仅既有 directory-fsync capability allowlist 可降级；`EIO`、permission、unknown 与 close failure fail closed，warning 不含 path。C-4P2A 不实现 C-5 trace/action/receipt/retry，不改 JSONL 策略，也不构成 multi-file transaction。
6. **workspace lesson style C-4P2B（`70afe1d`）：**仅 `applyLessonStyle()` 的 canonical `assets/lesson.css` publish 用 shared replace。保持 CSS path/content、style normalization/validation、workspace/scope checks 与 legacy `0666 & umask` create mode。只有 temp write → file fsync → close → rename → directory fsync/close 成功后，才 append `lesson_style_applied` lifecycle、touch/save registry 并返回成功。write/file-sync/file-close/rename failure 保留旧 CSS 且清理 temp；rename 后 directory fsync/close failure 不返回成功、不 append lifecycle 或 touch/save registry，完整新 CSS 可保留为 complete-but-unacknowledged，绝不自动 rollback。仅既有 directory-fsync capability allowlist 可降级，warning 不含 path；`EIO`、permission、unknown 与 close failure fail closed。它不改变 renderer 的独立 global settings write，也未实现 lesson-style correlation、settings transaction、retry、actionId 或 receipt；这些能力属于独立、未批准的 future style design。C-5H 是 mission-first，明确不覆盖 style。它不改 JSONL，也不构成 multi-file transaction。
7. **cross-workspace change-history C-4P3（`99bf6fe`）：**`<appDataRoot>/learning-changes/history.json` 使用 shared `replaceDurably()`，保持 JSON 格式/path、tolerant read、每 workspace 20 条 retention、`writeQueue` 串行化及 legacy `0666 & umask` create mode。只有 temp write → file fsync → close → rename → directory fsync/close 成功后，change audit 才可 release 被 history retention 淘汰的 checkpoint。write/file-sync/file-close/rename failure 保留旧 history 并清理 temp；rename 后 directory fsync/close failure 不确认成功，完整新 history 可留存，但不得 release checkpoint 或自动 rollback。仅既有 shared directory-fsync capability allowlist 可带无路径通用 warning 降级；`EIO`、`EACCES`、`EPERM`、unknown 与 close failure fail closed。此切片不改变 UI、IPC、retention policy 或 C-5 action/receipt/trace 语义。
8. **workspace session-open-state C-4P4（`f8ad99c`）：**`<workspaceRoot>/.agent-sessions/session-open-state.v1.json` 从手写 file-fsync + rename 迁入 shared `replaceDurably()`，补齐 directory fsync/close。保持 stable JSON bytes、integrity、64 KiB 上限、256 entries、root-exclusive queue、root containment + symlink preflight 与 `0600` mode。仅当 branch-open 需要创建、更新或修复 session-open-state sidecar 时，须 await temp write → file fsync → close → rename → directory fsync/close 成功，branch-open 才可成功；该写入路径失败不得返回成功。`shouldWrite=false` 的无写入 branch-open 仍可成功。对该写入路径，pre-rename failure 保留旧 sidecar 并清理 temp；rename 后 directory fsync/close failure fail closed，完整新 sidecar 可保留但不确认、不 rollback。仅 shared directory capability allowlist 可用无路径通用 warning 降级；`EIO`、permission、unknown 与 close failure fail closed。此切片不改变 UI、IPC、schema、audit、ledger、registry、trace、transaction 或 C-5 语义。
9. **workspace Markdown C-4P5（`278f141`）：**`TeachingWorkspaceDocuments.saveMarkdown()` 的 allowlisted workspace Markdown canonical publish 用 shared `replaceDurably()`。保持 allowed paths、path normalization/traversal 拒绝、parent containment + symlink preflight、final symlink 以 rename 替换而不跟随、metadata readback 及普通 `0666 & umask` mode。只有 document durable publish 成功后，service 才 touch/save registry；pre-rename failure 保留旧 bytes 并清理 temp。rename 后 directory fsync/close 的非 allowlist failure fail closed，完整新 Markdown 可留存、不 rollback，registry 不更新。仅 shared 五个 directory capability codes（`EOPNOTSUPP`、`ENOTSUP`、`ENOSYS`、`EINVAL`、`EISDIR`）可带无路径通用 warning 降级；其余错误 fatal。此切片不增加 backups、transactions、locks、queue、UI、IPC 或 C-5 trace。
10. **music cookies C-4P7（`0d55fd8`）：**MusicCookieStore 的私有登录 cookie 状态 `<userData>/music-cookies.json` 从裸 `writeFile` 迁入 shared `replaceDurably()`，显式 `0600`。保持 JSON schema、default path、normalization、tolerant initial read 和调用方 API；single-flight initial load 与 mutation queue 不变。`set`/`clear` 只有 durable success 才更新内存；durable failure 本次 fail closed，rename 后完整 bytes 可留但内存不确认，下一次 mutation 先 refresh canonical，若 refresh read 失败不得基于旧内存 publish。仅 shared directory capability allowlist（`EINVAL`、`ENOSYS`、`ENOTSUP`、`EOPNOTSUPP`、`EISDIR`）可无 path/cookie 通用 warning 降级；其它 directory I/O、permission、unknown 与 close failure fatal。此切片不引入 cookie backup、migration、transaction、trace、IPC/UI 或 schema change。
11. **learning-outcome settlement C-4P6 design gate（未实施）：**见 `docs/plans/local-data-learning-outcome-durable-settlement-design.md`。未来仅在批准同一 scope 内的 immutable record post-link parent-directory failure propagation、`outcome.json` owner/ledger-manifest writer lock、record-first controlled repair、shared durable capability 与 I/O injection seam 后，才可另立实现切片；若只采用 overwrite durable write，不得称 settlement durable closure，也不得称多文件事务。
12. 旧 `atomicWriteFile`（`src/main/teaching-workspace/lifecycle.ts:202-207`）继续保留为 compatibility helper；其它仍自行 `writeFile`/`rename` publish canonical artifact 的 writer 不因 review、archive、MISSION、lesson.css、change-history、session-open-state sidecar、allowlisted workspace Markdown 或 music cookie store 的迁移而自动视为 durable。每个 consumer 必须另立切片并证明其 bytes、mode/permission、legacy cleanup 与错误语义。C-3 的 `replaceWithBackup` 继续建在共享原语上，不能复制两套 fsync/rename 代码。
13. **C-4P9 session-audit durable append design gate（未实施、未批准）：**见 `docs/plans/local-data-session-audit-durable-append-design.md`。future implementation 仅可在获批的固定 audit-path、non-rotation durable append capability、per-path queue、row/torn-tail conflict contract、archive/ledger recovery 边界与 audit-specific I/O seam 内进行；不得直接使用 `appendDurableJsonlLine()` 的默认 month/size rotation，也不得把 C-4P1 或 C-5E 的代码/测试结果计作 C-4P9 完成。
...
- write/file-sync/file-close/rename 失败不得改写旧 canonical；directory sync/close 在 rename 后失败时不得报成功，且只允许既有 capability allowlist 降级。C-4P0/C-4P1/C-4P2A/C-4P2B/C-4P3/C-4P4/C-4P5/C-4P7 都不承诺多文件事务或 post-rename rollback。C-4P1 另须证明 JSON failure 不触发 Markdown/audit/ledger，Markdown failure 保留 JSON 且不触发 audit/ledger；C-4P2A/C-4P2B 分别须证明任何 durable failure 都不 append 相应 lifecycle 或 touch/save registry；C-4P3 须证明 failure 不 release retention checkpoint；C-4P4 须证明 sidecar durable failure 不让 branch-open 报成功；C-4P5 须证明 document durable failure 不 touch/save registry；C-4P7 须证明 `set`/`clear` 只在 durable success 后确认内存，failure 后 post-rename bytes 可留但本次 fail closed，下一次 mutation 需 canonical refresh，refresh read failure 不得基于旧内存 publish，且只有 shared allowlist 可用无 path/cookie warning 降级。C-4P6 尚无实现或测试结果；未来批准的同一 scope 必须以 I/O injection seam 验证 `outcome.json` 的 pre/post failure、allowlist、无-record marker idempotency、record-marker repair、immutable record link 后 directory failure blocking、指定 crash windows、corrupt state 及 schema/path/`0600` compatibility；未传播 immutable record post-link parent-directory failure 的切片只能称 overwrite adoption，不得称 settlement durable closure。
- consumer 覆盖必须分别记录；不得以 settings、registry、Memory、review、archive、MISSION、lesson.css、change-history、session-open-state sidecar 或 allowlisted workspace Markdown 或 music cookie store 的通过证明其它 writer 已迁移，也不得改变 learning-session ledger/native capability 边界。C-4P2B 也不得被用作 lesson-style correlation/settings transaction/retry/actionId/receipt 已实现证据；这些能力属于独立、未批准的 future style design，C-5H 是 mission-first 且不覆盖 style。
- C-4P0 实际验证：
...
```bash
pnpm exec vitest run --project unit tests/unit/teaching-workspace-lesson-style-durable.unit.test.ts tests/unit/durable-file.unit.test.ts --reporter=dot
pnpm run typecheck
pnpm run check:security
git diff --check
```

第一条是 direct targeted Vitest 形式，实际输出 **2 files / 24 tests passed**；不是 `pnpm run test:unit -- ...` 或全量 unit 声明。其余三项均 passed。

- C-4P3（`99bf6fe`）实际验证：

```bash
pnpm exec vitest run --project unit tests/unit/teaching-workspace-change-history-durable.unit.test.ts tests/unit/durable-file.unit.test.ts --reporter=dot
pnpm exec vitest run --project integration tests/integration/teaching-workspace-change-audit.integration.test.ts --reporter=dot
pnpm run typecheck
pnpm run check:security
git diff --check
```

前两条均为 direct targeted Vitest：unit 实际输出 **2 files / 26 tests passed**，integration 实际输出 **1 file / 3 tests passed**；两者均不是 `pnpm run test:unit -- ...` 或全量 suite 声明。typecheck、security 与 diff checks 均 passed。

- C-4P4（`f8ad99c`）实际验证：

```bash
pnpm exec vitest run --project unit tests/unit/agent-conversation-session-tree-durable.unit.test.ts tests/unit/agent-conversation-session-tree.unit.test.ts tests/unit/durable-file.unit.test.ts --reporter=dot
pnpm run typecheck
pnpm run check:security
git diff --check
```

第一条是 direct targeted Vitest，实际输出 **3 files / 38 tests passed**；不是 `pnpm run test:unit -- ...`、integration 或全量 suite 声明。typecheck、security 与 diff checks 均 passed。

- C-4P5（`278f141`）实际验证：

```bash
pnpm exec vitest run --project unit tests/unit/durable-file.unit.test.ts tests/unit/teaching-workspace-documents-durable.unit.test.ts --reporter=dot
pnpm exec vitest run --project integration tests/integration/teaching-workspace-documents.integration.test.ts --reporter=dot
pnpm run check:workspace-markdown
pnpm run typecheck
pnpm run check:security
git diff --check
```

前两条分别是 direct targeted Vitest unit 与 integration：unit 实际输出 **2 files / 26 tests passed**，integration 实际输出 **1 file / 4 tests passed**；不声称 full suite 或其它范围。`pnpm run check:workspace-markdown`、typecheck、security 与 diff checks 均 passed。

- C-4P7（`0d55fd8`）实际验证：

```bash
pnpm exec vitest run --project unit tests/unit/music-cookie-store-durable.unit.test.ts --reporter=dot
pnpm run typecheck
pnpm run check:security
git diff --check
```

第一条是 direct targeted Vitest unit，实际输出 **1 file / 19 tests passed**；不声称 full suite。typecheck、security 与 diff checks 均 passed。

### 回滚策略

由于 API/文件格式保持不变，已迁移 consumer 只能逐一评估回退，不能把 C-4P0、C-4P1、C-4P2A、C-4P2B、C-4P3、C-4P4、C-4P5、C-4P7 或任一已迁移路径的回退泛化为全局恢复旧 `atomicWriteFile`。不得回滚 ledger 自己的已存在 durability/locking 机制；失败清理不得删除 canonical 文件。
