...
> 说明：本文不是 ADR。涉及教学 authority、prompt-cache、skill manifest、IPC 或 settlement 的实现变更，落地前必须新增或更新 ADR，并链接到 [`docs/adr/README.md`](adr/README.md)。
...
对于 `teaching-site` 已定义的 hard gate，继续沿用 fail-closed 原则；对于非安全性的教学建议 gate，可以允许用户显式 override，但必须留下可见的工作区标记和诊断。
...
### Phase 2：host-owned registry 与纯 planner
...
| gate 过严导致体验僵硬 | 用户无法做 demo 或快速试验 | 区分硬安全 gate 与可显式 override 的产物 gate，并留下 audit marker |
...
1. **P0：修复 `teach` 可能因未安装而静默缺席的问题；**
2. **P0：通过 ADR 固定 Teaching Kernel 与 Skill Orchestrator 的 authority 边界；**
3. **P1：增加 host-owned skill taxonomy/registry 与纯 planner；**
4. **P1：prompt 改为按 stage 装配，而非多 skill 全量拼接；**
5. **P2：上线多选 chip、plan preview 和 preset；**
6. **P2：重写三个模板化通用教学 skill；**
7. **P3：在 ADR 后评估 manifest schema v2。**
