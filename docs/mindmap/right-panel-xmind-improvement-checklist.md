...
- **地图级外观：** `MindMapThemePanel.tsx` 支持背景 HEX、透明/清除、预置色、CJK-safe 字体、彩虹 palette 预览/切换以及关闭彩虹后的统一 `lineColor`；背景与字体明确标为整个文档作用域。**背景透明度与最近颜色（B-07）已完成**：新增 0–100% 不透明度滑杆（将背景改写为 8 位 `#RRGGBBAA`，透明时禁用）、最近使用背景色（localStorage，上限 8，可清除），每次提交均经 `document.apply-theme` command 保持 undo/redo 与 revisioned persistence；`mind-map-theme-panel.unit.test.tsx` 覆盖 8 位 HEX、透明度滑杆、禁用态、最近颜色记录/回放/清洗。
