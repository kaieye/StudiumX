...
### 3.0 `database` 分支实施审计（2026-07-18；C-4P0 `5c0dd96`、C-4P1 `34c48f4`、C-4P2A `b8eb3ab`、C-4P2B `70afe1d`、C-4P3 `99bf6fe`、C-4P4 `f8ad99c`、C-4P5 `278f141`、C-4P7 `0d55fd8`；C-5G 代码 `1bbdf7c`、测试补充 `e63e051`；C-6A `5803176`）

审计范围是 `main..database` 的原有八个数据提交及后续 C-5B `7a1ca7e`、C-5C `e849d51`、C-5D `dee70d6`、C-5E `d6a94a1`、C-5F `426eb6e`、C-5G 功能代码 `1bbdf7c` 与测试补充 `e63e051`，以及 C-4P0 `5c0dd96`、C-4P1 `34c48f4`、C-4P2A `b8eb3ab`、C-4P2B `70afe1d`、C-4P3 `99bf6fe`、C-4P4 `f8ad99c`、C-4P5 `278f141`、C-4P7 `0d55fd8` 与 C-6A `5803176`；`1bbdf7c` 是本次记录的**功能代码提交**，`e63e051` 是其后的**测试补充提交**，两者均不是后续文档提交后的当前 HEAD。“已实施”只表示下表所述切片已由相应代码提交实现，并由列出的测试与本次定向验证覆盖，**不把候选中的可选/破坏性扩展误记为完成**。
...
| C-4 | `ca73537`：共享 durable replace；`5c0dd96`（C-4P0）：review canonical `.studiumx/progress.json`；`34c48f4`（C-4P1）：conversation archive canonical JSON/Markdown；`b8eb3ab`（C-4P2A）：workspace `MISSION.md`；`70afe1d`（C-4P2B）：workspace `assets/lesson.css`；`99bf6fe`（C-4P3）：cross-workspace change-history `<appDataRoot>/learning-changes/history.json`；`f8ad99c`（C-4P4）：workspace `.agent-sessions/session-open-state.v1.json` sidecar；`278f141`（C-4P5）：TeachingWorkspaceDocuments allowlisted workspace Markdown canonical publish；`0d55fd8`（C-4P7）：MusicCookieStore 私有登录 cookie 状态 `<userData>/music-cookies.json`。已迁移的是共享原语的关键 consumer，**不是所有 writer**。 | `src/main/persistence/durable-file.ts:81-103, 214-312`；Memory record writer 在 `src/main/teaching-memory-catalog/record-file.ts`；review publish 在 `src/main/teaching-workspace/review.ts:54-73`；archive publish/order 在 `src/main/agent-conversation-archive.ts:57-125`；MISSION update 在 `src/main/teaching-workspace.ts:622-649`；lesson style publish 在 `src/main/teaching-workspace.ts:659-684`；C-4P3 history publish/retention checkpoint handoff 在 `src/main/teaching-workspace-change-history.ts:13-92`、`src/main/teaching-workspace-change-audit.ts:57-98`；C-4P4 session-open-state publish/root-exclusive queue/containment preflight 在 `src/main/agent-conversation-session-tree.ts:31-32, 650-730, 1108-1146, 1334-1348`；C-4P5 Markdown publish/preflight/readback 与 service registry handoff 在 `src/main/teaching-workspace-documents.ts:69-96, 221-273`、`src/main/teaching-workspace.ts:1770-1785`；C-4P7 cookie store durable publish/refresh boundary 在 `src/main/music/music-cookie-store.ts:50-162`。`tests/unit/durable-file.unit.test.ts`、`tests/unit/teaching-workspace-review-durable.unit.test.ts`、`tests/unit/agent-conversation-archive-durable.unit.test.ts`、`tests/unit/teaching-workspace-mission-durable.unit.test.ts`、`tests/unit/teaching-workspace-lesson-style-durable.unit.test.ts`、`tests/unit/teaching-workspace-change-history-durable.unit.test.ts`、`tests/unit/agent-conversation-session-tree-durable.unit.test.ts`、`tests/unit/teaching-workspace-documents-durable.unit.test.ts` 与 `tests/unit/music-cookie-store-durable.unit.test.ts` 覆盖 durable order、失败清理、权限/I/O fail-closed、mode、directory-fsync 降级；`tests/integration/teaching-workspace-change-audit.integration.test.ts` 另覆盖 history failure 不释放 retention checkpoint；archive 另覆盖 JSON → Markdown publish order 与 audit/ledger 成功、short-circuit 边界。 | 其它仍自行 `writeFile`/`rename` publish canonical artifact 的 writer 不因上述 consumer（包括 session-open-state sidecar 与 allowlisted workspace Markdown）的迁移而自动视为 durable，仍须逐一另立切片；C-4P7 不引入 cookie backup、migration、transaction、trace、IPC/UI 或 schema change；C-4P1 不改变 conversation audit 的既有普通 append 策略，也不改变 learning-work ledger 既有 durable JSONL append 的 file + directory fsync 策略；不支持平台仍仅按既定 capability 策略降级。C-4P6 learning-outcome durable settlement 仅登记 design gate：见 `docs/plans/local-data-learning-outcome-durable-settlement-design.md`；它是下一项待批准 follow-up，未迁移、未测试，更不构成 settlement durable closure。 C-4P8 workspace tool durable publish 也仅登记审查否决后的 design gate：见 `docs/plans/local-data-workspace-tool-durable-publish-design.md`；该 design gate 不记录任何 C-4P8 已实施或已通过的测试结果，不得把 `write_workspace_file` 或 C-4P5 allowlisted document service 视为已完成该迁移。 C-4P9 session-audit durable append 也仅登记独立 design gate：见 `docs/plans/local-data-session-audit-durable-append-design.md`；固定 per-conversation audit JSONL 必须保持单文件 non-rotating，C-4P1 的 ordinary append 不得直接迁移或改接 `durable-jsonl` 默认 rotation；该 design gate 未实施、未批准，不记录任何 C-4P9 代码或测试结果，也不得把 C-4P1 或 C-5E trace 视为已完成该迁移。**C-4P2B 未实现 lesson-style correlation、settings transaction、retry/actionId/receipt；它们属于独立、未批准的 future style design。C-5H 是 mission-first，明确不覆盖 style。** |
| C-5 | `55442ad`：conversation save trace；`7a1ca7e`：C-5B Memory CRUD trace；`e849d51`：C-5C learning-session trace；`dee70d6`：C-5D conversation lifecycle trace；`d6a94a1`：C-5E conversation audit JSONL trace；`426eb6e`：C-5F forked conversation trace；**C-5G 功能代码 `1bbdf7c`、测试补充 `e63e051`：workspace activation lifecycle trace**。C-5G 为 explicit create、activation bootstrap create 的 `workspace_created`，以及此前未注册 root 的首次 import 的 `workspace_imported` lifecycle JSONL event，在 main 内生成 UUID trace；不从 IPC、renderer 或 shared payload 接收。 | C-5E 新写 header/entry 只条件性写入 normalized lowercase UUID，malformed/secret-like 值不落盘；legacy no-trace/malformed header/entry tolerant read 且不改写，trace 不进入 audit ID/hash/parent/dedupe，version 保持 `1`。C-5F：returned persisted child record 的同一 trace 写入 child canonical、normal fork 的 learning-work snapshot 和 managed workspace lifecycle。C-5G：`src/main/teaching-workspace/activation-lifecycle.ts` 只在已有 event-emitting activation seam 透传 trace；它是 main-only、metadata-only 的 correlation metadata，不参与 workspace ID、root path、registry/index、目录命名、event ID/kind、dedupe、query/filter；material provision 与 workspace index 成功后才 append lifecycle。既有 root re-import 维持幂等：不追加 event、不产生新 trace，既有 JSONL bytes/trace 不变。沿用 lifecycle writer 的 normalize/安全持久化；不扫描、回填、迁移或重写历史 trace-free/malformed rows、registry/index 或用户可见文件。测试：`tests/unit/teaching-workspace-activation-lifecycle.unit.test.ts`、`tests/unit/teaching-workspace-lifecycle-jsonl.unit.test.ts`、`tests/integration/trace-propagation.integration.test.ts`；其中 direct test 以 `service.getState()` → `ensureRegistry()` 触发默认 bootstrap，直接验证 canonical trace、唯一 event、默认 workspace identity/scaffold，且第二次 load 的 `sessions.jsonl` 原始 bytes 不变。 | **learning-session ledger、saveAgentConversation lifecycle 子例、conversation audit JSONL、fork lifecycle trace，以及 workspace activation 的 create/import 子例已从 C-5 remaining queue 移除。**legacy source fork 继续跳过 shared learning-work ledger；source canonical JSON/Markdown/audit/shared ledger 不迁移、不回填、不重写，保持 bytes；global fork 仍不写 workspace lifecycle。仍未覆盖 `mission_updated`、`lesson_style_applied`、`lesson_generated` 等其它 lifecycle producers，及其它 user actions；这些写域需另行设计。**C-5H 只记录 mission-first action correlation 的[设计门槛](plans/local-data-workspace-user-mutation-correlation-design.md)，明确不覆盖 style；没有实现 mission trace 或 receipt，不能作为 C-5 completion 证据。lesson-style correlation、settings transaction、retry/actionId/receipt 属于独立、未批准的 future style design。** 本轮仅做 design discovery，没有业务代码/测试变更或测试结果。C-5E 不解决既有 audit read+append concurrency，日志仍是 tagged text，不是 JSON。 |
...
第一条是**直接的定向 Vitest** 命令，不是 `pnpm run test:unit -- ...` package-script 形式；实际输出为 **5 files / 42 tests passed**。typecheck、security 与 diff checks 均 passed。

**C-4P2B lesson style 验证（`70afe1d`）**：
...
第一条是**直接的定向 Vitest** 命令，实际输出为 **2 files / 24 tests passed**；不是 `pnpm run test:unit -- ...` 或全量 unit 声明。typecheck、security 与 diff checks 均 passed。

**C-4P3 change-history 验证（`99bf6fe`）**：
...
前两条均为**直接的定向 Vitest** 命令：unit 实际输出 **2 files / 26 tests passed**，integration 实际输出 **1 file / 3 tests passed**；两者均不是 package-script 或全量 suite 声明。typecheck、security 与 diff checks 均 passed。

**C-4P4 session-open-state 验证（`f8ad99c`）**：
...
第一条是**直接的定向 Vitest** 命令，实际输出 **3 files / 38 tests passed**；不是 `pnpm run test:unit -- ...`、integration 或全量 suite 声明。typecheck、security 与 diff checks 均 passed。

**C-4P5 workspace Markdown 验证（`278f141`）**：
...
前两条分别是**直接的定向 Vitest** unit 与 integration 命令：unit 实际输出 **2 files / 26 tests passed**，integration 实际输出 **1 file / 4 tests passed**；不声称 full suite 或其它范围。`pnpm run check:workspace-markdown`、typecheck、security 与 diff checks 均 passed。
...
C-5E (`d6a94a1`，代码提交) 的验证证据如下；结果只覆盖 conversation audit JSONL trace 子例：
...
C-5F (`426eb6e`，代码提交) 的验证证据如下；结果只覆盖 forked conversation trace 子例，**不**宣告其它 lifecycle producers、其它 user actions、C-2 retention、C-1 FTS/query 或 C-6 controlled migration 已完成：
...
C-5G（功能代码 `1bbdf7c`；测试补充 `e63e051`）的独立验证证据如下；direct test 以 `service.getState()` → `ensureRegistry()` 触发默认 bootstrap，直接验证 canonical trace、唯一 event、默认 workspace identity/scaffold，以及第二次 load 的 `sessions.jsonl` 原始 bytes 不变：
...
这些验证仅覆盖 workspace activation 的 create/import trace 子例，**不**表示其它 lifecycle/user actions、C-2 retention、C-1 FTS/query 或 C-6 controlled migration 已完成。
...
**已实施的粒度**：`ca73537` 提供共享 `replaceDurably()`；settings/registry/index 的关键状态路径与 Memory record writer 已按各自切片接入。`5c0dd96` 再将 review 的 canonical `.studiumx/progress.json` 接入：保持 JSON schema、canonical path 和 legacy `0666 & umask` create-mode 契约；只有 temp write → file fsync → close → rename → directory fsync/close 完成后才向调用方返回更新后的 progress。`34c48f4`（C-4P1）再将 conversation archive 的 canonical JSON 与 Markdown 接入：正常顺序严格为 **JSON durable publish → Markdown durable publish → audit append → existing learning-work ledger append**；两个 canonical 文件不是 transaction，也不做 rollback。JSON durable failure 阻止 Markdown、audit 和 ledger；Markdown durable failure 保留已完成 JSON，仍阻止 audit/ledger。它保持 collision preflight、trace 与 C-7 redaction/legacy nonmutating 语义，并保留新文件 `0666 & umask` mode。`b8eb3ab`（C-4P2A）仅将 workspace `updateMission()` 的 canonical `MISSION.md` 更新接入：保持 render/path、输入清理与既有 root/scope checks、legacy `0666 & umask` mode；只有 durable publish 完成后才 append `mission_updated` lifecycle、touch/save registry 并返回成功。write/file-sync/file-close/rename failure 保留旧 canonical；rename 后 directory fsync/close failure 仍 fail closed、不写 lifecycle/registry，完整新文件可保留为未确认（complete-but-unacknowledged），不自动 rollback。`70afe1d`（C-4P2B）同样仅将 `applyLessonStyle()` 的 canonical `assets/lesson.css` publish 接入：保持 CSS path/content、style validation、workspace/scope checks 与 legacy `0666 & umask` mode；temp write → file fsync → close → rename → directory fsync/close 成功后才 append `lesson_style_applied` lifecycle、touch/save registry 并返回成功。pre-rename failure 保留旧 CSS 并清理 temp；rename 后 directory fsync/close failure fail closed、不写 lifecycle/registry，完整新 CSS 可保留为未确认且不自动 rollback。两者仅允许既有 directory-fsync capability allowlist 以无路径通用 warning 降级；权限、`EIO`、unknown 与 close error 都不报成功。C-4P2B 不改变 renderer 的独立 global settings write，也未实现 lesson-style correlation/settings transaction/retry/actionId/receipt；这些能力属于独立、未批准的 future style design。C-5H 是 mission-first，明确不覆盖 style。C-4P2B 不改 JSONL，也不构成 multi-file transaction。`99bf6fe`（C-4P3）再将 cross-workspace `<appDataRoot>/learning-changes/history.json` 接入 shared `replaceDurably()`：保持 JSON 格式、路径、tolerant read、每 workspace 20 条上限、`writeQueue` 串行化与 legacy `0666 & umask` mode。只有 durable history append 成功后 change audit 才会 release 被 retention 淘汰的 checkpoint；pre-rename failure 保留旧 history 并清理 temp；rename 后 directory fsync/close failure fail closed，完整新 history 可留存但不确认、不 release checkpoint、不 rollback。仅共享 directory capability allowlist 可用无路径通用 warning 降级；不新增 UI、IPC、retention policy 或 C-5 语义。`f8ad99c`（C-4P4）将 `<workspaceRoot>/.agent-sessions/session-open-state.v1.json` 从手写 file-fsync + rename 迁入 shared `replaceDurably()`，补齐 directory fsync/close：保持 stable JSON bytes、integrity、64 KiB 上限、256 entries、root-exclusive queue、root containment + symlink preflight 与 `0600` mode。pre-rename failure 保留旧 sidecar 并清理 temp；rename 后 directory fsync/close failure fail closed，完整新 sidecar 可保留但不 rollback，branch-open 不得报成功。只有 shared directory capability allowlist 可用无路径通用 warning 降级；`EIO`、权限、unknown 与 close failure 均 fail closed。它不改变 UI、IPC、schema、audit、ledger、registry、trace、transaction 或 C-5 语义。`278f141`（C-4P5）将 `TeachingWorkspaceDocuments.saveMarkdown()` 的 allowlisted workspace Markdown canonical publish 接入 shared `replaceDurably()`：保留 allowed paths、path normalization/traversal 拒绝、parent containment + symlink preflight、final symlink 以 rename 替换而不跟随、metadata readback，以及普通 `0666 & umask` mode。只有 document durable publish 成功后，service 才 touch/save registry。pre-rename failure 保留旧 bytes 并清理 temp；rename 后 directory fsync/close 的非 allowlist failure fail closed，完整新 Markdown 可留存、不 rollback，registry 不更新。仅 shared 的五个 directory capability codes（`EOPNOTSUPP`、`ENOTSUP`、`ENOSYS`、`EINVAL`、`EISDIR`）可发出无路径通用 warning 后降级，其余错误均 fatal。此切片不增加 backups、transactions、locks、queue、UI、IPC 或 C-5 trace。`0d55fd8`（C-4P7）将 MusicCookieStore 的私有登录 cookie 状态 `<userData>/music-cookies.json` 从裸 `writeFile` 迁入 shared `replaceDurably()`，显式 `0600`：保持 JSON schema、default path、normalization、tolerant initial read 与调用方 API。single-flight initial load 与 mutation queue 保持；`set`/`clear` 仅在 durable success 后更新内存。任何 durable failure 均 fail closed；rename 后完整 bytes 可留但内存不确认，下一次 mutation 必须先 refresh canonical，refresh read 失败时不得基于旧内存 publish。仅 shared directory capability allowlist（`EINVAL`、`ENOSYS`、`ENOTSUP`、`EOPNOTSUPP`、`EISDIR`）可发出不含 path/cookie 的通用 warning 后降级；其它 directory I/O、permission、unknown 与 close failure 均 fatal。它不引入 cookie backup、migration、transaction、trace、IPC/UI 或 schema change。

**仍未实施**：这不是“一处改、全局提升”。review、archive、`MISSION.md`、`lesson.css`、change-history、session-open-state sidecar、allowlisted workspace Markdown 与 music cookie store 是已迁移的部分 consumer；其它仍自行 `writeFile`/`rename` publish canonical artifact 的 writer 仍须逐 consumer 另立切片。C-4P1 不改变 conversation audit 的既有普通 append 策略，也不改变 learning-work ledger 既有 durable JSONL append 的 file + directory fsync 策略。C-4P6 仅已记录 learning-outcome durable settlement 的 design gate（`docs/plans/local-data-learning-outcome-durable-settlement-design.md`）：其 authority、ordered publish、post-link failure propagation、repair 与 lock 前置决策仍待批准；没有 migration、测试或 settlement durable closure 已完成。lesson-style correlation、settings transaction、retry/actionId/receipt 仍属于独立、未批准的 future style design；C-5H 是 mission-first，明确不覆盖 style。
...
1. **C-5 trace 后续设计**：conversation、Memory CRUD、learning-session ledger、`saveAgentConversation()` 的 `agent_conversation_recorded` workspace lifecycle 子例、conversation audit JSONL、forked conversation，以及 workspace activation 的 explicit/bootstrap create 与首次 import 子例已分别由 `55442ad`、`7a1ca7e`、`e849d51`、`dee70d6`、`d6a94a1`、`426eb6e`、`1bbdf7c` 覆盖。C-5H 已在[workspace user-mutation correlation 设计门槛](plans/local-data-workspace-user-mutation-correlation-design.md)中记录 mission-first 的候选 action-id/receipt contract，**仅为 design discovery，尚未获批或实现，且明确不覆盖 style**。C-5I 已在[direct-UI lesson generation lifecycle / user-action correlation 设计门槛](plans/local-data-lesson-generation-user-action-correlation-design.md)中记录 direct UI `generateLesson` / stream 的候选 action-id/receipt/recovery contract，**同样仅为 design discovery，尚未获批或实现，且明确排除 agent `generate_lesson`、generic workspace writer、style 与全局 transaction/UI redesign**。仍需为 **其它 lifecycle producers（包括 `mission_updated`、`lesson_style_applied`、`lesson_generated`）与其它 user actions**（需设计）另行定义写域、trusted identity、retry/idempotency 和安全日志边界；不要回写历史 source，也不得因此声称全量 trace 覆盖。
