...
| Runtime turn 中间态（未 durable） | 进程内存 / 未来可选 runtime store | 内存 | **是** | ADR-0052 **Proposed / 未实施**；**当前未授权** SQLite runtime SoT |
...
| runtime session store | **DB-OPT-6 Design-complete / unimplemented** — ADR-0052 Proposed |
...
| 可以做 runtime SQLite 吗？ | 仅 ADR-0052 实现授权后；且文件仍是 export/resume 权威 |
...
```markdown
### Database acceptance gates (ADR-0053)
...
P2 boundary check (see ADR-0053 §3):
...
- **可选 runtime session store**（仅性能缓存；export/resume 仍以文件为准）——见 ADR-0052，**须独立实现授权**，本项 won't-do **不**默许实现。
...
| | |
| --- | --- |
| **状态** | **未授权实现**；DB-OPT-6 Design-complete / unimplemented — [ADR-0052](0052-runtime-session-store.md) Proposed；**无生产 schema/writer** |
| **允许讨论的形状** | 高 churn 运行时状态的 disposable SQLite **缓存**；turn 成功路径 durable 仍落文件；库可删并由文件重建 |
| **硬门槛** | 实现须遵守 ADR-0052；通过 §2；**不得**用本项覆盖 won't-do 写权威迁库 |
...
| 项 | 来源 | 原因 / 闸 |
| --- | --- | --- |
| SQLite 作为会话/教学**写权威** | Marvis/ZCode | DB-P2-3 won't do；runtime cache 另案 ADR-0052 |
...
| **DB-P0-3** | Usage/observability 可选 projection | **Done**（最小集） | `usage-ledger.ts` + `usage_projection`；设计权威 ADR-0051 |
...
| ID | 项 | 状态 | 落点摘要 |
| --- | --- | --- | --- |
| **DB-P1-1** | Token/tool/turn usage 细粒度 ledger | **设计 Done + 最小实现 via P0-3** | ADR-0051 |
...
| **DB-OPT-6** | runtime session store 设计 ADR | **Design-complete / unimplemented** → ADR-0052 |
...
- **设计 only：** ADR-0052 runtime store（实现须另案 + §2 + 不得覆盖 DB-P2-3）。
...
| Usage ledger 设计 | ADR-0051 |
| Runtime store 设计 only | ADR-0052 |
