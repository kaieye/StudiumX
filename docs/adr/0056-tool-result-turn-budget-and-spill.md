...
- **证据：** `src/main/ai/tools/tool-result-budget.ts`、`src/main/ai/agent-loop.ts`、`src/main/teaching-conversation-runtime.ts` / `src/main/ai/delegation-runtime.ts` / `src/main/ai/lesson-plan-production.ts`（上下文传入）、`tests/unit/tool-result-budget.unit.test.ts`
...
- 不把 spill 文件纳入 support-bundle 自动打包策略的完整产品面（路径规则与脱敏可后续叠加 [ADR-0034](0034-redacted-support-bundle.md)）。
