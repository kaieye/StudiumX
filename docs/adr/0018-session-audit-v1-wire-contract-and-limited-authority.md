# ADR-0018：Session-audit V1 wire contract 与有限 authority

- **状态：** 已实施（audit 专用 V1 wire / identity / exact-retry append；不代表 C-4P9 complete，也不授权 generic JSONL）
...
本决定记录 audit 专用 V1 contract 与 S2 production append 边界；完整 C-4P9 platform/operations close-out 仍未关闭。
...
- 本 ADR **不**关闭 C-4P9，不证明 cross-process exclusion、Windows native/power-loss、完整 host capability matrix 或 operations runbook closure。
