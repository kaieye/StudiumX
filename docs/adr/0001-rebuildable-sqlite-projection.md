...
- 本 ADR **不**授权将会话正文主写权威迁入 SQLite（见 DB-P2-3 与 [ADR-0052](0052-runtime-session-store.md) 边界；0052 仅为 Proposed / 未实施设计）。
- 本 ADR **不**授权可选 runtime session store 的生产 schema/writer；讨论见 ADR-0052，实现须另案且文件仍是 export/resume 权威。
