...
- **后续切片：** main-only readonly dry-run intent/receipt 与 destructive 延期见 [ADR-0022](0022-memory-readonly-migration-dry-run-and-destructive-deferral.md)。
...
没有 copy、checksum verify、explicit confirmation、delete legacy、迁移 UI / 新 IPC command、启动迁移、后台迁移或自动 resume。Readonly preflight 不构成 destructive authorization。main-only readonly dry-run intent/receipt preview 与 destructive migration 延期见 [ADR-0022](0022-memory-readonly-migration-dry-run-and-destructive-deferral.md)；不得将 preflight 或 dry-run 复用为 consent。
