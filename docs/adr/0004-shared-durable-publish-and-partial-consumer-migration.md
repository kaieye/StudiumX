...
## C-4P6-S111：well-formed settlement-marker record relativePath mismatch 的 tests-only evidence

`ede6d29`（`test(data): cover marker record path mismatch and non-array evidence residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 writing kind 但 `marker.record.relativePath` 与 canonical session path 不一致 residual：`normalizeMarker` identity check 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S112。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S111/S112 marker record relativePath mismatch + non-array evidence residuals: 1 file, 137 tests passed
```

## C-4P6-S112：well-formed non-array settlement-marker evidenceEventIds 的 tests-only evidence

`ede6d29` 同提交补齐 settled 后 well-formed 但 `evidenceEventIds` 为 non-array residual：`normalizeMarker` via `stringArray` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。
...
## C-4P6-S115：well-formed non-string settlement-marker evidenceEventIds item 的 tests-only evidence

`cb5c780`（`test(data): cover non-string marker evidence item and unknown kind residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 well-formed 但 `evidenceEventIds` 含 non-string item residual：`normalizeMarker` via `stringArray` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S116。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S115/S116 non-string marker evidence item + unknown kind residuals: 1 file, 141 tests passed
```

## C-4P6-S116：well-formed unknown settlement-marker kind 的 tests-only evidence

`cb5c780` 同提交补齐 settled 后 well-formed 但 unknown non-null `kind` residual：`normalizeMarker` via `outcomeKind` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S116 unknown settlement-marker kind residual: 1 file, 141 tests passed
```

## C-4P6-S117：well-formed missing settlement-marker record contentSha256 key 的 tests-only evidence

`c5fe645`（`test(data): cover missing marker record sha and whitespace evidence residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 writing kind 但 `marker.record` 缺 `contentSha256` key residual：`normalizeMarker` via `normalizeRecordRef` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S118。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S117/S118 missing marker record sha key + whitespace evidence item residuals: 1 file, 143 tests passed
```

## C-4P6-S118：well-formed whitespace-only settlement-marker evidenceEventIds item 的 tests-only evidence

`c5fe645` 同提交补齐 settled 后 well-formed 但 `evidenceEventIds` 含 whitespace-only item residual：`normalizeMarker` via `stringArray` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S118 whitespace-only settlement-marker evidenceEventIds item residual: 1 file, 143 tests passed
```

## C-4P6-S119：well-formed zero settlement-marker evaluatorVersion 的 tests-only evidence

`b6dab25`（`test(data): cover zero marker evaluatorVersion and missing path residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 well-formed 但 settlement marker `evaluatorVersion: 0` residual：`normalizeMarker` via `number()` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S120。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S119/S120 zero marker evaluatorVersion + missing record relativePath residuals: 1 file, 145 tests passed
```

## C-4P6-S120：well-formed missing settlement-marker record relativePath key 的 tests-only evidence

`b6dab25` 同提交补齐 settled 后 writing kind 但 `marker.record` 缺 `relativePath` key residual：`normalizeMarker` via `normalizeRecordRef` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。
...
## C-4P6-S123：well-formed non-writing settlement-marker kind with record 的 tests-only evidence

`e62a962`（`test(data): cover non-writing kind with record and missing schema residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 non-writing `kind: needs_practice` 但 non-null `record` residual：`normalizeMarker` record-presence check 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S124。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S123/S124 non-writing kind with record + missing schemaVersion residuals: 1 file, 149 tests passed
```

## C-4P6-S124：well-formed missing settlement-marker schemaVersion key 的 tests-only evidence

`e62a962` 同提交补齐 settled 后 otherwise well-formed 但缺 `schemaVersion` key residual：`normalizeMarker` schemaVersion 检查拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S124 missing settlement-marker schemaVersion key residual: 1 file, 149 tests passed
```

## C-4P6-S125：well-formed null settlement-marker schemaVersion 的 tests-only evidence

`9d47153`（`test(data): cover null marker schemaVersion and missing evidence residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 well-formed 但 `schemaVersion: null` residual：`normalizeMarker` schemaVersion 检查拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S126。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S125/S126 null marker schemaVersion + missing evidenceEventIds residuals: 1 file, 151 tests passed
```

## C-4P6-S126：well-formed missing settlement-marker evidenceEventIds key 的 tests-only evidence

`9d47153` 同提交补齐 settled 后 well-formed 但缺 `evidenceEventIds` key residual：`normalizeMarker` via `stringArray` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S126 missing settlement-marker evidenceEventIds key residual: 1 file, 151 tests passed
```

## C-4P6-S127：well-formed Windows device-name settlement-marker sessionId 的 tests-only evidence

`8dedd2d`（`test(data): cover device-name marker sessionId and missing outcomeId residuals`）修改 `tests/unit/learning-outcome-committer.unit.test.ts`，补齐 settled 后 well-formed 但 `sessionId: "CON"` residual：`text()` 成功后 `requireLearningSessionId` / `isLearningSessionId` 因 Windows device-name ban 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。同提交亦引入 S128。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S127/S128 device-name marker sessionId + missing outcomeId residuals: 1 file, 153 tests passed
```

## C-4P6-S128：well-formed missing settlement-marker outcomeId key 的 tests-only evidence

`8dedd2d` 同提交补齐 settled 后 well-formed 但缺 `outcomeId` key residual：`normalizeMarker` via `text()` 拒绝；restart `reconcile()` → `review_required` + `invalid_settlement_marker`；不 rewrite authority、不 evaluate；同 operation commit → `conflict/review_required`。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S128 missing settlement-marker outcomeId key residual: 1 file, 153 tests passed
```

## C-4P6-S129：durable stage write fail-closed residual 的 tests-only evidence

`be90d6c`（`test(data): cover stage write and sync fail-closed residuals`）仅修改测试，补齐 ordered publication 中 `durableStage` **write** 失败 residual：注入 stage `write` `EIO` 后 `commit` → `retryable_failure/reconciliation_required`；不 publish immutable record / outcome / marker；session 保持 active。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S129 stage write residual: 1 file, 155 tests passed
```

## C-4P6-S130：durable stage sync fail-closed residual 的 tests-only evidence

`be90d6c` 同提交补齐 ordered publication 中 `durableStage` **sync** 失败 residual：注入 stage `sync` `EIO` 后 `commit` → `retryable_failure/reconciliation_required`；不 publish immutable record / outcome / marker；session 保持 active。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S130 stage sync residual: 1 file, 155 tests passed
```

## C-4P6-S131：durable stage open fail-closed residual 的 tests-only evidence

`9721a0d`（`test(data): cover stage open and close fail-closed residuals`）仅修改测试，补齐 ordered publication 中 `durableStage` **open** 失败 residual：注入 stage `open:wx` `EIO` 后 `commit` → `retryable_failure/reconciliation_required`；不 publish immutable record / outcome / marker；session 保持 active。无生产语义改动。

```bash
pnpm exec vitest run --project unit tests/unit/learning-outcome-committer.unit.test.ts
# P6-S131 stage open residual: 1 file, 157 tests passed
```

## C-4P6-S132：durable stage close fail-closed residual 的 tests-only evidence

`9721a0d` 同提交补齐 ordered publication 中 `durableStage` **close** 失败 residual：注入 stage `close` `EIO` 后 `commit` → `retryable_failure/reconciliation_required`；不 publish immutable record / outcome / marker；session 保持 active。无生产语义改动。
...
## C-4P6-S135：durable record-parent mkdir EIO fail-closed residual 的 tests-only evidence

`d2dfbd3`（`test(data): cover record-parent mkdir EIO and ENOSPC fail-closed residuals`）仅修改测试，补齐 ordered publication 中 `publishImmutable` learning-records parent **mkdir** `EIO` residual：预创建 stage 目录后注入 `mkdir:${recordsDirectory}` `EIO`；`commit` → `retryable_failure/reconciliation_required`；stage 保留，不 publish record/outcome/marker；session 保持 active。无生产语义改动。

```text
# P6-S135 record-parent mkdir EIO residual: 1 file, 161 tests passed
```

## C-4P6-S136：durable record-parent mkdir ENOSPC fail-closed residual 的 tests-only evidence

`d2dfbd3` 同提交补齐 ordered publication 中 `publishImmutable` learning-records parent **mkdir** `ENOSPC` residual：预创建 stage 目录后注入 `mkdir:${recordsDirectory}` `ENOSPC`；`commit` → `retryable_failure/reconciliation_required`；stage 保留，不 publish record/outcome/marker；session 保持 active。无生产语义改动。

```text
# P6-S136 record-parent mkdir ENOSPC residual: 1 file, 161 tests passed
```


## C-4P6-S137：durable outcome temp open fail-closed residual 的 tests-only evidence

`ac1e204`（`test(data): cover outcome temp open and write fail-closed residuals`）仅修改测试，补齐 `replaceDurably(outcome.json)` temp **open** `EIO` residual：immutable record 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；outcome/marker 未 publish；session 保持 active。无生产语义改动。

```text
# P6-S137 outcome temp open residual: 1 file, 163 tests passed
```

## C-4P6-S138：durable outcome temp write fail-closed residual 的 tests-only evidence

`ac1e204` 同提交补齐 `replaceDurably(outcome.json)` temp **write** `EIO` residual：immutable record 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；outcome/marker 未 publish；session 保持 active。无生产语义改动。

```text
# P6-S138 outcome temp write residual: 1 file, 163 tests passed
```


## C-4P6-S139：durable outcome temp sync fail-closed residual 的 tests-only evidence

`03d0284`（`test(data): cover outcome temp sync and close fail-closed residuals`）仅修改测试，补齐 `replaceDurably(outcome.json)` temp **sync** `EIO` residual：immutable record 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；outcome/marker 未 publish；session 保持 active。无生产语义改动。

```text
# P6-S139 outcome temp sync residual: 1 file, 165 tests passed
```

## C-4P6-S140：durable outcome temp close fail-closed residual 的 tests-only evidence

`03d0284` 同提交补齐 `replaceDurably(outcome.json)` temp **close** `EIO` residual：immutable record 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；outcome/marker 未 publish；session 保持 active。无生产语义改动。

```text
# P6-S140 outcome temp close residual: 1 file, 165 tests passed
```


## C-4P6-S141：durable marker temp open fail-closed residual 的 tests-only evidence

`188b4d8`（`test(data): cover marker temp open and write fail-closed residuals`）仅修改测试，补齐 `replaceDurably(outcome-settlement.json)` temp **open** `EIO` residual：record/outcome/completed manifest 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；marker 未 publish。无生产语义改动。

```text
# P6-S141 marker temp open residual: 1 file, 167 tests passed
```

## C-4P6-S142：durable marker temp write fail-closed residual 的 tests-only evidence

`188b4d8` 同提交补齐 `replaceDurably(outcome-settlement.json)` temp **write** `EIO` residual：record/outcome/completed manifest 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；marker 未 publish。无生产语义改动。

```text
# P6-S142 marker temp write residual: 1 file, 167 tests passed
```


## C-4P6-S143：durable marker temp sync fail-closed residual 的 tests-only evidence

`a66c5ee`（`test(data): cover marker temp sync and close fail-closed residuals`）仅修改测试，补齐 `replaceDurably(outcome-settlement.json)` temp **sync** `EIO` residual：record/outcome/completed manifest 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；marker 未 publish。无生产语义改动。

```text
# P6-S143 marker temp sync residual: 1 file, 169 tests passed
```

## C-4P6-S144：durable marker temp close fail-closed residual 的 tests-only evidence

`a66c5ee` 同提交补齐 `replaceDurably(outcome-settlement.json)` temp **close** `EIO` residual：record/outcome/completed manifest 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；marker 未 publish。无生产语义改动。

```text
# P6-S144 marker temp close residual: 1 file, 169 tests passed
```



## C-4P6-S145：durable outcome post-rename directory open fail-closed residual 的 tests-only evidence

`e6af9b9`（`test(data): cover post-rename directory open fail-closed residuals`）仅修改测试，补齐 `replaceDurably(outcome.json)` rename 成功后 session directory **open** `EIO` residual：immutable record 与 outcome 已 publish；`commit` → `retryable_failure/reconciliation_required`；manifest 保持 active 且 `outcomeRef: null`；marker 未 publish。无生产语义改动。

```text
# P6-S145 outcome post-rename directory open residual: 1 file, 171 tests passed
```

## C-4P6-S146：durable marker post-rename directory open fail-closed residual 的 tests-only evidence

`e6af9b9` 同提交补齐 `replaceDurably(outcome-settlement.json)` rename 成功后 session directory **open** `EIO` residual：record/outcome/completed manifest/marker 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；session 已 completed。无生产语义改动。

```text
# P6-S146 marker post-rename directory open residual: 1 file, 171 tests passed
```



## C-4P6-S147：durable marker post-rename directory sync fail-closed residual 的 tests-only evidence

`020f78a`（`test(data): cover marker post-rename directory sync/close residuals`）仅修改测试，补齐 `replaceDurably(outcome-settlement.json)` rename 成功后 session directory **sync** `EIO` residual：record/outcome/completed manifest/marker 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；session 已 completed。无生产语义改动。

```text
# P6-S147 marker post-rename directory sync residual: 1 file, 173 tests passed
```

## C-4P6-S148：durable marker post-rename directory close fail-closed residual 的 tests-only evidence

`020f78a` 同提交补齐 `replaceDurably(outcome-settlement.json)` rename 成功后 session directory **close** `EIO` residual：record/outcome/completed manifest/marker 已存在后 fail closed；`commit` → `retryable_failure/reconciliation_required`；session 已 completed。无生产语义改动。

```text
# P6-S148 marker post-rename directory close residual: 1 file, 173 tests passed
```



## C-4P6-S149：ledger complete after_state_loaded fail-closed residual 的 tests-only evidence

`4910654`（`test(data): cover manifest complete fail-closed ledger residuals`）仅修改测试，补齐 ordered publication 中 **outcome publish 之后、manifest complete 路径** 的 `after_state_loaded`（`operation: complete`）fault residual：record/outcome 已存在；`commit` → `retryable_failure/reconciliation_required`；manifest 保持 active 且 `outcomeRef: null`；marker 未 publish。无生产语义改动。

```text
# P6-S149 manifest complete after_state_loaded residual: 1 file, 175 tests passed
```

## C-4P6-S150：ledger complete after_stage_sync fail-closed residual 的 tests-only evidence

`4910654` 同提交补齐 **manifest complete 路径** 的 `after_stage_sync`（`operation: complete`，`.manifest-stage-*`）fault residual：record/outcome 已存在；`commit` → `retryable_failure/reconciliation_required`；manifest 保持 active；未完成的 `.manifest-stage-*` residual 可保留；marker 未 publish。无生产语义改动。

```text
# P6-S150 manifest complete after_stage_sync residual: 1 file, 175 tests passed
```



## C-4P6-S151：ledger complete after_file_stat pre-write fail-closed residual 的 tests-only evidence

`9461405`（`test(data): cover complete after_file_stat fail-closed residuals`）仅修改测试，补齐 ordered publication 中 **outcome publish 之后、manifest complete 写前** 的 `after_file_stat`（`operation: complete`，`outcome.json`）fault residual：record/outcome 已存在；`commit` → `retryable_failure/reconciliation_required`；manifest 保持 active 且 `outcomeRef: null`；marker 未 publish。无生产语义改动。

```text
# P6-S151 complete after_file_stat pre residual: 1 file, 177 tests passed
```

## C-4P6-S152：ledger complete after_file_stat post-write fail-closed residual 的 tests-only evidence

`9461405` 同提交补齐 **completed manifest 写后二次 validateCommittedOutcome** 的 `after_file_stat`（`operation: complete`，`outcome.json`）fault residual：record/outcome/completed manifest 已存在；`commit` → `retryable_failure/reconciliation_required`；marker 未 publish。无生产语义改动。

```text
# P6-S152 complete after_file_stat post residual: 1 file, 177 tests passed
```

## C-4P6-S153：ledger complete after_file_stat on session.json load fail-closed residual 的 tests-only evidence
...
```text
# P6-S153 complete after_file_stat session.json residual: 1 file, 179 tests passed
```

## C-4P6-S154：ledger complete after_file_stat on session event load fail-closed residual 的 tests-only evidence
...
```text
# P6-S154 complete after_file_stat event residual: 1 file, 179 tests passed
```

## C-4P6-S155：commit pre-write after_writer_lock_acquired fail-closed residual 的 tests-only evidence
...
```text
# P6-S155 pre-write after_writer_lock_acquired residual: 1 file, 181 tests passed
```

## C-4P6-S156：commit pre-write repair after_file_stat on session.json fail-closed residual 的 tests-only evidence
...
```text
# P6-S156 pre-write repair after_file_stat residual: 1 file, 181 tests passed
```

## C-4P6-S157：commit pre-write lagging eventCount projection conflict residual 的 tests-only evidence
...
```text
# P6-S157 pre-write lagging eventCount residual: 1 file, 183 tests passed
```

## C-4P6-S158：commit pre-write advanced eventCount projection conflict residual 的 tests-only evidence
...
```text
# P6-S158 pre-write advanced eventCount residual: 1 file, 183 tests passed
```

## C-4P6-S159：commit pre-write active session non-null outcomeRef residual 的 tests-only evidence
...
```text
# P6-S159 pre-write active outcomeRef residual: 1 file, 185 tests passed
```

## C-4P6-S160：commit pre-write session version-behind-canonical-facts residual 的 tests-only evidence
...
```text
# P6-S160 pre-write version-behind residual: 1 file, 185 tests passed
```

## C-4P6-S161：commit pre-write active session non-null completedAt residual 的 tests-only evidence
...
```text
# P6-S161 pre-write active completedAt residual: 1 file, 187 tests passed
```

## C-4P6-S162：commit pre-write malformed session.json residual 的 tests-only evidence
...
```text
# P6-S162 pre-write malformed session.json residual: 1 file, 187 tests passed
```

## C-4P6-S163：commit pre-write completed session missing completedAt residual 的 tests-only evidence
...
```text
# P6-S163 pre-write completed missing completedAt residual: 1 file, 189 tests passed
```

## C-4P6-S164：commit pre-write unsupported session schemaVersion residual 的 tests-only evidence
...
```text
# P6-S164 pre-write unsupported schemaVersion residual: 1 file, 189 tests passed
```

## C-4P6-S165：commit pre-write completed session missing outcomeRef residual 的 tests-only evidence
...
```text
# P6-S165 pre-write completed missing outcomeRef residual: 1 file, 191 tests passed
```

## C-4P6-S166：commit pre-write session updatedAt-precedes-createdAt residual 的 tests-only evidence
...
```text
# P6-S166 pre-write updatedAt-precedes-createdAt residual: 1 file, 191 tests passed
```

## C-4P6-S167：commit pre-write session completedAt-before-createdAt residual 的 tests-only evidence
...
```text
# P6-S167 pre-write completedAt-before-createdAt residual: 1 file, 193 tests passed
```

## C-4P6-S168：commit pre-write invalid session status residual 的 tests-only evidence
...
```text
# P6-S168 pre-write invalid session status residual: 1 file, 193 tests passed
```

## C-4P6-S169：commit pre-write session.json directory residual 的 tests-only evidence
...
```text
# P6-S169 pre-write session.json directory residual: 1 file, 195 tests passed
```

## C-4P6-S170：commit pre-write session.json non-file symlink residual 的 tests-only evidence
...
```text
# P6-S170 pre-write session.json non-file symlink residual: 1 file, 195 tests passed
```

## C-4P6-S171：commit pre-write session id-directory mismatch residual 的 tests-only evidence
...
```text
# P6-S171 pre-write session id-directory mismatch residual: 1 file, 197 tests passed
```

## C-4P6-S172：commit pre-write invalid canonical identity flags residual 的 tests-only evidence
...
```text
# P6-S172 pre-write invalid canonical identity flags residual: 1 file, 197 tests passed
```

## C-4P6-S173：commit pre-write session completedAt-after-updatedAt residual 的 tests-only evidence
...
```text
# P6-S173 pre-write completedAt-after-updatedAt residual: 1 file, 199 tests passed
```

## C-4P6-S174：commit pre-write non-object session.json residual 的 tests-only evidence
...
```text
# P6-S174 pre-write non-object session.json residual: 1 file, 199 tests passed
```

## C-4P6-S175：commit pre-write conversationRefs-not-array residual 的 tests-only evidence
...
```text
# P6-S175 pre-write conversationRefs-not-array residual: 1 file, 201 tests passed
```

## C-4P6-S176：commit pre-write unknown session manifest key residual 的 tests-only evidence
...
```text
# P6-S176 pre-write unknown session manifest key residual: 1 file, 201 tests passed
```

## C-4P6-S177：commit pre-write malformed durable session event JSON residual 的 tests-only evidence
...
```text
# P6-S177 pre-write malformed durable session event JSON residual: 1 file, 203 tests passed
```

## C-4P6-S178：commit pre-write durable session event filename/eventId mismatch residual 的 tests-only evidence
...
```text
# P6-S178 pre-write durable session event filename/eventId mismatch residual: 1 file, 203 tests passed
```

## C-4P6-S179：commit pre-write non-object durable session event JSON residual 的 tests-only evidence
...
```text
# P6-S179 pre-write non-object durable session event JSON residual: 1 file, 205 tests passed
```

## C-4P6-S180：commit pre-write durable session event sessionId mismatch residual 的 tests-only evidence
...
```text
# P6-S180 pre-write durable session event sessionId mismatch residual: 1 file, 205 tests passed
```

## C-4P6-S181：commit pre-write unknown session events-directory entry residual 的 tests-only evidence
...
```text
# P6-S181 pre-write unknown session events-directory entry residual: 1 file, 207 tests passed
```

## C-4P6-S182：commit pre-write durable session event path-as-directory residual 的 tests-only evidence
...
```text
# P6-S182 pre-write durable session event path-as-directory residual: 1 file, 207 tests passed
```

## C-4P6-S183：commit pre-write durable session event unknown-key residual 的 tests-only evidence
...
```text
# P6-S183 pre-write durable session event unknown-key residual: 1 file, 209 tests passed
```

## C-4P6-S184：commit pre-write durable session event path non-file symlink residual 的 tests-only evidence
...
```text
# P6-S184 pre-write durable session event path non-file symlink residual: 1 file, 209 tests passed
```

## C-4P6-S185：commit pre-write durable session event unsupported schemaVersion residual 的 tests-only evidence
...
```text
# P6-S185 pre-write durable session event unsupported schemaVersion residual: 1 file, 211 tests passed
```

## C-4P6-S186：commit pre-write durable session event unsupported kind residual 的 tests-only evidence
...
```text
# P6-S186 pre-write durable session event unsupported kind residual: 1 file, 211 tests passed
```

## C-4P6-S187：commit pre-write durable session event non-object payload residual 的 tests-only evidence
...
```text
# P6-S187 commit pre-write durable session event non-object payload residual: 1 file, 213 tests passed
```

## C-4P6-S188：commit pre-write durable session event malformed present traceId residual 的 tests-only evidence
...
```text
# P6-S188 commit pre-write durable session event malformed present traceId residual: 1 file, 213 tests passed
```

## C-4P6-S189：commit pre-write durable session event invalid sequence residual 的 tests-only evidence

`16ed880`（`test(data): cover pre-write event sequence and stage-entry residuals`）补齐 **任何 durable publish 之前** durable session event invalid `sequence` residual：`parseEvent` fail closed → `conflict/review_required`；不调用 evaluator；poison event bytes 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S189 commit pre-write durable session event invalid sequence residual: 1 file, 215 tests passed
```

## C-4P6-S190：commit pre-write session event staging non-file entry residual 的 tests-only evidence

`16ed880` 同提交补齐 **任何 durable publish 之前** `.event-stage-*` non-file staging entry residual：`readSessionEvents` fail closed → `conflict/review_required`；不调用 evaluator；unsafe staging directory residual 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S190 commit pre-write session event staging non-file entry residual: 1 file, 215 tests passed
```

## C-4P6-S191：commit pre-write durable session event invalid recordedAt residual 的 tests-only evidence

`8b14419`（`test(data): cover pre-write event recordedAt and occurredAt residuals`）补齐 **任何 durable publish 之前** durable session event invalid `recordedAt` residual：`parseEvent` fail closed → `conflict/review_required`；不调用 evaluator；poison event bytes 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S191 commit pre-write durable session event invalid recordedAt residual: 1 file, 217 tests passed
```

## C-4P6-S192：commit pre-write durable session event invalid occurredAt residual 的 tests-only evidence

`8b14419` 同提交补齐 **任何 durable publish 之前** durable session event invalid `occurredAt` residual：`normalizeEventInput` fail closed → `conflict/review_required`；不调用 evaluator；poison event bytes 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S192 commit pre-write durable session event invalid occurredAt residual: 1 file, 217 tests passed
```

## C-4P6-S193：commit pre-write durable session event malformed turnId residual 的 tests-only evidence

`c1fb162`（`test(data): cover pre-write event turnId and sequence-gap residuals`）补齐 **任何 durable publish 之前** durable session event malformed present `turnId` residual：`normalizeEventInput` fail closed → `conflict/review_required`；不调用 evaluator；poison event bytes 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S193 commit pre-write durable session event malformed turnId residual: 1 file, 219 tests passed
```

## C-4P6-S194：commit pre-write durable session event non-contiguous sequence residual 的 tests-only evidence

`c1fb162` 同提交补齐 **任何 durable publish 之前** durable session event non-contiguous `sequence` residual：`readSessionEvents` contiguity check fail closed → `conflict/review_required`；不调用 evaluator；poison event bytes 原样保留；record/outcome/marker 均为 `ENOENT`；无 durable publish residual。无生产语义改动。

```text
# P6-S194 commit pre-write durable session event non-contiguous sequence residual: 1 file, 219 tests passed
```
