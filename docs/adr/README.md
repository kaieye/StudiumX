...
| [ADR-0173](0173-mind-map-schema-v2-and-revisioned-repository.md) | 思维导图 schema v2 / revisioned repository / 统一 command / 互通保真 | **已采纳（M0 基线契约，实施分 M1 起步）**：schemaVersion 2 + revision/expectedRevision CAS（禁止 last-write-wins）；统一 `MindMapCommand` reducer + undo/redo、active sheet 显式入命令上下文；XMind 导入/导出返回 `preserved / approximated / dropped / warnings` 兼容性报告、禁止静默丢字段、导入安全预算；来源锚点只读教学投影、导图不是教学权威。基线见 docs/mindmap/m0-baseline.md 与 docs/mindmap/benchmarks.md。 |
