...
- runtime registry 以真实 descriptor-relative durable capability 为 gate：可用 host 才暴露 `write_workspace_file`；Windows 和其它不可用 host 返回稳定 availability `{ available: false, code: 'containment_unavailable', message: '当前平台无法安全发布工作区文件。' }` 并根本不注册 write definition/handler。只读 workspace tools 仍可用，三个 `settings.tools.approvalMode` 均不会产生一个注定失败的写入审批；不使用旧 `workspaceWritePermission` 运行时字段。
