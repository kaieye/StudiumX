...
| [ADR-0172](0172-mind-map-and-ai-assist.md) | 原生思维导图与 AI 辅助生成 | **已采纳（设计见 docs/mindmap/design.md，实施中）**：StudiumX 原生导图编辑与 AI 辅助生成（复用 provider 基建，Zod 校验）；对话语言明确命名的 Markdown 仅在主进程受限读取并作为 provider-only 资料，无文件选择器、无全工作区上传；导图是用户内容非教学权威；durable 工作区写；Markdown / OPML 导入导出与自绘 SVG 画布。 |
| [ADR-0173](0173-mind-map-schema-v2-and-revisioned-repository.md) | 思维导图 schema v2 / revisioned repository / 统一 command | **已采纳（M0 基线契约，实施分 M1 起步）**：schemaVersion 2 + revision/expectedRevision CAS（禁止 last-write-wins）；统一 `MindMapCommand` reducer + undo/redo、active sheet 显式入命令上下文；只支持 Markdown / OPML 导入导出与 SVG / PNG 导出；来源锚点只读教学投影、导图不是教学权威。基线见 docs/mindmap/m0-baseline.md 与 docs/mindmap/benchmarks.md。 |
