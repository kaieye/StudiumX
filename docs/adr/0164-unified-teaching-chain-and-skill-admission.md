...
- **实施状态：** complete
- **日期：** 2026-07-28
- **范围：** 收紧正式教学的准入与产品心智：一条统一 authority lifecycle、exactly-one Teaching Kernel、host-owned Skill admission、primary strategy / workflow-router 基数，以及 Capability Library 产品面。
- **取代：** 无
- **被取代：** 无
...
- **证据：** host eligibility projection / planner 准入逻辑与 `tests/unit/teaching-ipc-commands.unit.test.ts`、`tests/unit/skill-orchestration-preview.unit.test.ts`；builtin skill 治理见 `src/main/.../builtin-skill-orchestration-policy.ts`。
...
正式教学选择面与 host planner 的 formal admission 一致；unknown ID 在 planner 中 fail-closed。Kernel exactly one 且不作为普通 chip；current-stage prompt 只加载 active bodies 与 Kernel。同一输入、canonical facts 和 continuity projection 得到稳定 plan；每个 teaching stage 至多一个 active primary strategy。artifact lead-writer conflict 与多 verifier `parallel_readonly` 行为不回归。preview、diagnostics 和 continuity 仍是可重建投影，不成为 canonical teaching truth。
...
- 不把 raw capability / slash 或 IPC 8 项上限解释为自由拼装教学策略的承诺。
- 不让 personal/custom Skill 获得 Kernel、primary strategy 或 settlement writer 身份。
- 不改变 settlement sole-writer、effect lattice、approval、`expectedRevision`、`toolsReplayed:false` 或无默认远程外发。
- 不引入第二 learner/mastery authority 或重写 TeachingTurnCoordinator / AgentRun / LearningSessionLedger。