...
export type PetSpriteAtlasStyle = {
...
const idleFrames: readonly PetAnimationFrame[] = [
...
const stateFrames: Record<PetVisualState, readonly PetAnimationFrame[]> = {
  idle: idleFrames,
...
export function getPetSpriteRowCount(appearance: PetAppearanceId = 'boba'): number {
  return getPetDefinition(appearance).spriteVersionNumber === 2 ? 11 : STANDARD_ROW_COUNT
}

export function getPetAnimationFrames(state: PetVisualState): readonly PetAnimationFrame[] {
...
export function getPetSpriteAtlasStyle(
  appearance: PetAppearanceId,
  state: PetVisualState,
  frame: number
): PetSpriteAtlasStyle {
  const rowCount = getPetSpriteRowCount(appearance)
  const x = (frame / (FRAME_COUNT - 1)) * 100
  const y = (getPetSpriteRow(state) / (rowCount - 1)) * 100
  return {
...
export function getPetSpriteDisplayHeight(width: number): number {
  return Math.round((width * CELL_HEIGHT) / CELL_WIDTH)
}
