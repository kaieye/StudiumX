...
/**
 * The narrow orchestration boundary for durable workspace mutation audits.
 * Git checkpoint construction and diff calculation remain implementation
 * details; callers only capture, record, list, and read an audit.
 */
export class TeachingWorkspaceChangeAudit {
  private readonly history: TeachingWorkspaceChangeHistoryStore
  ...
  async capturePreMutation(workspaceRoot: string): Promise<TeachingWorkspaceChangeSnapshot> {
    return captureWorkspaceChangeSnapshot(workspaceRoot)
  }

  async recordCompletedMutation(options: {
    workspaceId: string
    ...
    const summary = await summarizeWorkspaceChanges(options)
    if (!summary) return null

    // Remember the persisted set before appending so checkpoint refs for entries
    // evicted by retention can be removed without guessing from timestamps.
    const beforeRetention = await this.history.list(options.workspaceId)
    await this.history.append(options.workspaceId, summary)
    const retainedIds = new Set((await this.history.list(options.workspaceId)).map((entry) => entry.id))
    await Promise.all(beforeRetention
      .filter((entry) => !retainedIds.has(entry.id))
      .map((entry) => releaseWorkspaceChangeCheckpoint(entry).catch(() => {})))
    return summary
  }

  async listSummaries(workspaceId: string): Promise<TeachingWorkspaceChangeSummary[]> {
    return this.history.list(workspaceId)
  }

  async readSelectedDiff(options: {
    workspaceId: string
    workspaceRoot: string
    relativePath: string
    changeId?: string
    ...
    const selected = options.changeId
      ? await this.history.get(options.workspaceId, options.changeId)
      : await this.history.latest(options.workspaceId)
...
export async function releaseWorkspaceChangeCheckpoint(summary: TeachingWorkspaceChangeSummary): Promise<void> {
  if (!summary.checkpoint) return
  const key = createHash('sha256').update(summary.id).digest('hex').slice(0, 24)
  const prefix = `refs/studiumx/checkpoints/${key}`
  await Promise.all([
    executeGitCommand(summary.checkpoint.repositoryRoot, ['update-ref', '-d', `${prefix}/before`]),
    executeGitCommand(summary.checkpoint.repositoryRoot, ['update-ref', '-d', `${prefix}/after`])
  ])
}
...
function fileKindForPath(relativePath: string): TeachingWorkspaceChangedFileKind {
  ...
  if (path === '.studiumx/index.json') return 'workspace_index'
  if (path === '.studiumx/sessions.jsonl') return 'session_event'
