...
<body>
  <main class="${cls.page}">
    <header class="${cls.hero}">
      <p class="${cls.heroKicker}">Lesson ${escapeHtml(lesson.id)} · ${escapeHtml(String(plan.durationMinutes))} min</p>
...
<body>
  <main class="${cls.page} reference-page">
    <header class="${cls.hero}">
      <p class="${cls.heroKicker}">Reference · Lesson ${escapeHtml(lesson.id)}</p>
...
function renderQuizCard(item: LessonQuizItem): string {
  ...
  const choices = item.choices
    .map(
      (choice, index) =>
        `            <button type="button" ${data.quizChoice}="${letterFor(index)}">${escapeHtml(choice)}</button>`
    )
    .join('\n')
...
function renderMarkdown(source: string): string {
  ...
  const flushList = (): void => {
    if (list) {
      blocks.push(list === 'ul' ? `<ul class="${cls.compactList}">${listItems.join('')}</ul>` : `<ol>${listItems.join('')}</ol>`)
      list = null
      listItems = []
    }
  }
