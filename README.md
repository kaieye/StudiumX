...
- **本地优先、显式授权**：不默认启用工作区 shell，不默认上传遥测数据；涉及写入、外部写入和特权操作的工具都要经过 effect 分类与审批策略。
...
应用不会默认开启远程遥测或工作区 shell。工具按 `read`、`workspace_write`、`external_write`、`privileged` 分类；未知工具会失败关闭，写入及高风险操作由审批策略控制。MCP 设置仅提供连接的列表、编辑、导入与 OAuth 流程，密钥和令牌不会出现在公开 DTO、Doctor 输出或支持包中。
