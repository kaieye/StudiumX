...
5. **Tool effect authorization** — tools are classified (`read` | `workspace_write` | `external_write` | `privileged`) and fail closed for unknown tools (ADR-0024). Capability catalog remains fail-closed (ADR-0022). Workspace commands (`run_workspace_command` / `shell`, ADR-0152/0153) require `tools.enabled` (default **off**); when tools are on, `workspaceShell` defaults **on** (may be disabled). Commands are cwd-fenced, dual-axis with `sandboxMode` × `approvalMode` (需批准 / 按风险 / 本课放行 — never labeled YOLO). **Qualified product readiness is not claimed until** [agent-shell-sandbox-delivery-roadmap.md](docs/agent-shell-sandbox-delivery-roadmap.md) Stages A–F complete; current state is provisional (main-path projection / safelist hardening may still be open).
...
- StudiumX **does not** claim a fully qualified general shell / arbitrary code execution product path until delivery roadmap Stages A–F are complete. Provisional registry/policy code may exist (ADR-0152/0153); **no** YOLO labels; **no** Docker/VM-class OS isolation claim for local commands.
