...
- **MCP multi-source / auto-connect / workspace-root / plugin foundation** are authorized under ADR-0132 + ADR-0141; **Settings marketplace UI is out of current shipping surface** (ADR-0142 §6 — design non-claim, not a permanent ban; re-open path with preconditions) (parity with mainstream MCP clients). Prefer clear toggles and revoke/emergency paths over permanent feature bans. Auto-connect discovers tools; tool invocation still uses effect/approval. YOLO / always-approve labels remain forbidden.
...
- MCP Settings product surface (marketplace UI currently not shipped — design non-claim, re-open path in ADR-0142 §6): `docs/adr/0142-mcp-product-surface-settings-only.md`
