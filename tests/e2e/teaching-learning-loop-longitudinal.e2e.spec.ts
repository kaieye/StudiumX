...
test.describe('P0 longitudinal Electron Golden — learning-outcome commit loop', () => {
  test('wrong → needs_practice, correct → saved, replay + restart stay single-record @p0', async (
    {},
    testInfo: TestInfo
  ) => {
    ...
    const runtime: TestRuntime = await createTestRuntime(
      `${testInfo.project.name}-longitudinal-${testInfo.workerIndex}`
    )
    let launched = await launchElectronRuntime(runtime, testInfo)
    let failed = false

    try {
      let mainWindow = await firstWindow(launched.application)

      const created = await createWorkspaceViaApi(mainWindow)
      const publication = await seedFixedPreviewLesson({
        id: created.id,
        name: created.name,
      ...
      const seeded: SeededWorkspace = {
        ...created,
        sessionId: publication.lesson.sessionId,
        lessonRelativePath: publication.lesson.relativePath,
        lessonTitle: publication.lesson.title,
        sessionName: publication.lesson.sessionName
      }

      await refreshRendererFromMain(mainWindow, seeded.id)
      await openSeededLesson(mainWindow, {
        lessonTitle: seeded.lessonTitle,
        sessionName: seeded.sessionName
      })

      // 1) Wrong choice → needs_practice, no formal record
      await clickQuizChoice(mainWindow, 'a')
      await expect(mainWindow.locator('[data-learning-outcome-commit="needs_practice"]')).toBeVisible({
        timeout: 30_000
      })
      await expect(mainWindow.locator('[data-learning-outcome-commit="needs_practice"]')).toContainText(
        '还需要继续练习，不会记为掌握。'
      )
      expect(await countLearningRecords(seeded.rootPath)).toBe(0)
      expect(await readOutcomeJson(seeded.rootPath, seeded.sessionId)).toBeNull()

      // 2) Correct choice → saved / misconception_corrected, one record + outcome.json
      await clickQuizChoice(mainWindow, 'b')
      await expect(mainWindow.locator('[data-learning-outcome-commit="saved"]')).toBeVisible({
        timeout: 30_000
      })
      await expect(mainWindow.locator('[data-learning-outcome-commit="saved"]')).toContainText(
        '本次学习进展已保存'
      )
      await expect.poll(async () => countLearningRecords(seeded.rootPath), { timeout: 15_000 }).toBe(1)

      const outcome = await readOutcomeJson(seeded.rootPath, seeded.sessionId)
      ...
      expect(await countLearningRecords(seeded.rootPath)).toBe(1)
      const outcomeAfterRestart = await readOutcomeJson(seeded.rootPath, seeded.sessionId)
      expect(outcomeAfterRestart).toMatchObject({
        kind: 'misconception_corrected'
      })
      ...
      expect(replayAfterRestart).toMatchObject({
        status: 'already_committed',
        recordSaved: true
      })
      expect(await countLearningRecords(seeded.rootPath)).toBe(1)
      ...
      await launched.close({ failed: failed || testInfo.status !== testInfo.expectedStatus }).catch(() => undefined)
