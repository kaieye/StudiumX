...
describe('database PR gates documentation contract (ADR-0053)', () => {
  const policy = readRepo('docs/adr/0053-database-layered-authority-and-pr-gates.md')
  ...
  it('implements six-gate checklist with PR copy block', () => {
    ...
    expect(policy).toContain('Database acceptance gates (ADR-0053)')
    expect(policy).toContain('P2 boundary check')
    expect(policy).toContain('DB-P2-3')
  })

  it('points contributors and PR authors at ADR-0053 without authorizing P2 implementations', () => {
    expect(contributing).toContain('Database PR gates')
    expect(contributing).toContain('0053-database-layered-authority-and-pr-gates.md')
    expect(contributing).toMatch(/won'?t do/i)

    expect(prTemplate).toContain('Database-gates')
    expect(prTemplate).toContain('0053-database-layered-authority-and-pr-gates.md')
    ...
    expect(adrReadme).toContain('0053-database-layered-authority-and-pr-gates.md')
  })
  ...
  it('records honest P0/P1/OPT closeout and open-slice stance', () => {
    ...
    expect(policy).toContain('ADR-0052')
  })
})
