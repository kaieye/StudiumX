...
describe('useTodayAnalyticsSync', () => {
  ...
  it('stays inert (no client calls, empty peers) while analytics sync is not enabled', async () => {
  ...
  it('uploads and polls immediately, then on the 60s interval while enabled', async () => {
    const { putAnalyticsSummary, getAnalyticsPeersToday, createClient } = createClientHarness()
  ...
  it('stops uploading on a 403 upload gate but keeps polling, and surfaces the blocked flag', async () => {
    const { putAnalyticsSummary, getAnalyticsPeersToday, createClient } = createClientHarness()
    putAnalyticsSummary.mockRejectedValue(
      new SyncApiError(403, { error: { code: 'analytics_upload_disabled' } })
    )
    getAnalyticsPeersToday.mockResolvedValue({ peers: [PEER], asOf: '2026-08-01T12:00:00.000Z' })
    enableSync()
    ...
    expect(putAnalyticsSummary).toHaveBeenCalledTimes(1)
    expect(getAnalyticsUploadBlocked()).toBe(true)

    await act(async () => {
      await vi.advanceTimersByTimeAsync(60_000)
    })
    expect(putAnalyticsSummary).toHaveBeenCalledTimes(1) // stopped
    expect(getAnalyticsPeersToday).toHaveBeenCalledTimes(2) // poll continues
  })

  it('retries on network errors instead of stopping', async () => {
    const { putAnalyticsSummary, getAnalyticsPeersToday, createClient } = createClientHarness()
  ...
  it('clears peers and stops the loops when consent is toggled off mid-session', async () => {
    const { putAnalyticsSummary, getAnalyticsPeersToday, createClient } = createClientHarness()
    getAnalyticsPeersToday.mockResolvedValue({ peers: [PEER], asOf: '2026-08-01T12:00:00.000Z' })
    enableSync()
