...
describe('Lesson artifact publisher integration', () => {
  it('publishes the default-course lesson only and returns its durable paths with working asset links', async () => {
    ...
    expect(published.paths.recordRelativePath).toBeNull()
  ...
  it('keeps named-course placement and publishes Reference, Learning record, and review artifacts together', async () => {
    ...
    expect(published.eventPaths).toEqual([
      ...
      'courses/javascript-runtime/lesson/0002-event-loop-mechanics.md',
    ...
    await expect(readFile(published.paths.recordAbsolutePath!, 'utf8')).resolves.toContain('## 判定')
