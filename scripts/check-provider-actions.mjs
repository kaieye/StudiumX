...
assert.match(settingsView, /const activeProviderProbePayload = \{[\s\S]*baseUrl: activeModelSettingsProvider\.baseUrl[\s\S]*apiKey: activeModelSettingsProvider\.apiKey[\s\S]*endpointFormat: activeModelSettingsProvider\.endpointFormat[\s\S]*\} satisfies ProbeProviderPayload/)
assert.match(settingsView, /const result = await onProbeProvider\(activeProviderProbePayload\)/)
assert.match(settingsView, /const result = await onListUpstreamModels\(activeProviderProbePayload\)/)
assert.match(settingsView, /role="status" aria-live="polite"/)
assert.match(settingsView, /const \[apiKeyVisible, setApiKeyVisible\]/)
assert.match(settingsView, /type=\{apiKeyVisible \? 'text' : 'password'\}/)
assert.match(settingsView, /apiKeyVisible \? <EyeOff size=\{15\} \/> : <Eye size=\{15\} \/>/)

assert.match(settingsView, /onClick=\{\(\) => void onOpenExternal\(activeModelSettingsProvider\.docsUrl\)\}/)
assert.match(settingsView, /disabled=\{isCustomModelProvider \|\| !activeModelSettingsProvider\.docsUrl\}/)
assert.match(settingsView, /onClick=\{\(\) => void onOpenExternal\(activeModelSettingsProvider\.apiKeyUrl\)\}/)
assert.match(settingsView, /disabled=\{isCustomModelProvider \|\| !activeModelSettingsProvider\.apiKeyUrl\}/)

assert.match(settingsView, /const resetProvider = \{ \.\.\.preset, apiKey: activeModelSettingsProvider\.apiKey \}/)
assert.match(settingsView, /await onUpdateSettings\(\{[\s\S]*provider: \{[\s\S]*activeProviderId: resetProvider\.id[\s\S]*providers[\s\S]*\}[\s\S]*generator: \{[\s\S]*providerId: resetProvider\.id[\s\S]*model: resetProvider\.models\[0\] \?\? ''[\s\S]*endpointFormat: resetProvider\.endpointFormat[\s\S]*\}/)
assert.match(settingsView, /setProviderStatus\(t\('model\.statusReset'\)\)/)
assert.doesNotMatch(settingsView, /updateProvider\(\{ \.\.\.preset, apiKey: activeModelSettingsProvider\.apiKey \}\)/)
