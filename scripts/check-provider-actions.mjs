...
assert.match(main, /ipcMain\.handle\(teachingInvokeChannels\.listUpstreamModels, async \(_, payload: unknown\) => \{[\s\S]*const request = parseListUpstreamModelsPayload\(payload, settings\.provider\.providers\)[\s\S]*return fetchUpstreamModels\(request, resolveProxyUrl\(settings\)\)/)
...
assert.match(main, /ipcMain\.handle\(teachingInvokeChannels\.openExternal[\s\S]*return openExternalHttpUrl\(rawUrl, settings, \(url\) => shell\.openExternal\(url\)\)/)
