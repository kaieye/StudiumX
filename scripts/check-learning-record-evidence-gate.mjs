...
assert.match(source, /if \(!writesLearningRecord\(settlement\.marker\.kind\)\)/, 'Needs-practice and not-evidenced outcomes must take the no-record path.')
assert.match(tests, /outcome: \{ kind: 'needs_practice' \}, record: null/, 'Integration coverage must prove needs_practice does not publish a record.')
