...
try {
  ...
  assert.deepEqual(result.clientA.counts, ['2', '2', '1'], 'first client should see two people in the study space')
  assert.deepEqual(result.clientB.counts, ['2', '2', '1'], 'second client should see two people in the study space')
  ...
  assert.equal(result.clientA.stageHeading, '学习空间', 'first client should render the unified study space heading')
  assert.equal(result.clientB.stageHeading, '学习空间', 'second client should render the unified study space heading')
  assert.equal(result.clientA.legacyRoomCopyVisible, false, 'first client should not show legacy room/category copy')
  assert.equal(result.clientB.legacyRoomCopyVisible, false, 'second client should not show legacy room/category copy')
  ...
  assert.equal(result.clientA.roomTabCount, 0, 'first client should not render a room navigator')
  assert.equal(result.clientB.roomTabCount, 0, 'second client should not render a room navigator')
...
async function verifyTwoStudyClients(port, rootUrl, timeout) {
  ...
  while (Date.now() - startedAt < timeout) {
    ...
    if (
      ...
      && lastA.roomTabCount === 0
      && lastB.roomTabCount === 0
    ) break
  }
  ...
  const result = await client.send('Runtime.evaluate', {
    ...
    expression: `(() => {
      ...
      return {
        ...
        stageHeading: q('.study-cinema-topbar h2')?.textContent?.trim() ?? '',
        ...
        legacyRoomCopyVisible: /静音自习室|冲刺教室|深度学习舱|考试模拟间|专注契约|选择房间|在线房间|房间规则|同步房间|本房间/.test(text),
