import assert from 'node:assert/strict'
import { readFile } from 'node:fs/promises'

const app = await readFile('src/renderer/src/App.tsx', 'utf8')
const appStore = await readFile('src/renderer/src/app-shell/appStore.ts', 'utf8')
const contextTransitions = await readFile('src/renderer/src/app-shell/contextTransitions.ts', 'utf8')
const navigator = await readFile('src/renderer/src/app-shell/teaching-workspace-navigator.tsx', 'utf8')
const agentConversationRunner = await readFile('src/renderer/src/app-shell/agent-conversation-runner.ts', 'utf8')

assert.doesNotMatch(
  app,
  /我想先学习如何把 teach 技能包的 MISSION、RESOURCES 和 lessons 组织成一个 Electron 桌面应用的 MVP/,
  'overview composer should not prefill the old StudiumX MVP learning prompt'
)

assert.match(
  app,
  /\{\(view === 'overview' \|\| view === 'agent'\) && \(\s*<>\s*<OverviewChat active=\{active\} \/>/s,
  'overview teaching/chat surface should render the agent chat flow instead of a local lesson clarification flow'
)

assert.match(
  app,
  /const isTeachingMode = view !== 'agent' && overviewDialogMode === 'teaching'/,
  'OverviewChat should know when the switch is in teaching mode'
)

assert.match(
  app,
  /if \(item\.id === 'overview'\) \{\s*openWorkspaceTeachingMode\(\)\s*return\s*\}/,
  'new conversation navigation should route through the app shell context transition'
)

assert.match(
  contextTransitions,
  /function selectCourseFolderContext[\s\S]*overviewDialogMode:\s*'teaching'/,
  'opening a course folder should switch the composer back to teaching mode'
)

assert.match(
  contextTransitions,
  /const hasCourseContent = selectedCourseRelativePath[\s\S]*selectedCourse\.sessionCount > 0[\s\S]*view: hasCourseContent \? 'lessons' : 'overview'/,
  'empty course folders should open the teaching dialog instead of the empty lesson library'
)

// Course-tree open intents live in TeachingWorkspaceNavigator so App stays a
// thin wiring shell. Any course-tree row must still force teaching mode, and
// the workspace root must enter the teaching dialog before folder toggle.
assert.match(
  navigator,
  /const handleOpen = async \(\): Promise<void> => \{\s*if \(treeRoot === 'courses'\) onSetOverviewDialogMode\('teaching'\)/,
  'opening any row in the course tree should switch the composer back to teaching mode'
)

assert.match(
  app,
  /onSetOverviewDialogMode=\{setOverviewDialogMode\}/,
  'App must wire the navigator course-tree teaching-mode switch into the store'
)

assert.match(
  contextTransitions,
  /function openWorkspaceTeaching[\s\S]*\.\.\.openTeachingConversation\(\)[\s\S]*selectedCourseRelativePath:\s*null/,
  'opening a workspace folder should enter the overview teaching dialog, not stay in chat mode'
)

assert.match(
  navigator,
  /if \(isWorkspaceFolder\) \{\s*await onEnsureWorkspaceSelected\(\)[\s\S]*?onSelectFolder\(workspace\.id, node\.relativePath\)\s*onOpenWorkspaceTeachingMode\(\)\s*onToggle\(workspace\.id, node\.relativePath\)/,
  'clicking the imported workspace root folder should activate teaching mode before toggling the folder'
)

assert.match(
  app,
  /onOpenWorkspaceTeachingMode=\{openWorkspaceTeachingMode\}/,
  'App must wire workspace-root teaching activation into the navigator'
)

assert.match(
  contextTransitions,
  /function openLessonReaderContext[\s\S]*overviewDialogMode:\s*'teaching'/,
  'opening a course HTML file should keep the composer in teaching mode'
)

assert.match(
  contextTransitions,
  /function openAgentConversationContext[\s\S]*const isTeachingConversation = Boolean\(conversationCourseRelativePath\)[\s\S]*view:\s*isTeachingConversation \? 'overview' : 'agent'[\s\S]*overviewDialogMode:\s*isTeachingConversation \? 'teaching' : input\.currentOverviewDialogMode/,
  'opening a course conversation should enter the overview teaching dialog, not the normal chat view'
)

assert.match(
  contextTransitions,
  /function restorePendingConversationContext[\s\S]*view:\s*pending\.mode === 'teaching' \? 'overview' : 'agent'/,
  'restoring a pending teaching conversation should keep the teaching dialog mode'
)

// One brain: every free-form teaching input goes through the conversation
// agent, which owns clarification and calls generate_lesson when ready. The
// old regex-gated hand-off state must stay dead.
assert.doesNotMatch(
  app,
  /pendingLessonClarificationPrompt/,
  'the lesson clarification hijack state must not come back — teaching replies belong to the conversation'
)

assert.match(
  app,
  /const submitTeachingPrompt = \(value:\s*string,\s*attachments\?: AgentChatImageAttachment\[\]\):\s*void => \{[\s\S]*?void agentChat\(prompt,\s*\{\s*mode: 'teaching',\s*skillIds:/,
  'teaching-mode submit should always continue the teaching conversation'
)

assert.match(
  app,
  /const submitToTeachingAgent = \(rawInput = taskPrompt\) => \{[\s\S]*?openTeachingConversationView\(\)[\s\S]*?void agentChat\(prompt, \{ mode: 'teaching', skillIds \}\)/,
  'the lesson composer should route free-form prompts into the teaching conversation'
)

assert.match(
  app,
  /submitTeachingPrompt\(rawInput, attachments\)/,
  'teaching-mode submit should keep using the teaching submit handler'
)

// generate_lesson completion is owned by the conversation runner, then
// surfaced through the store callback — not by inlining done.generatedLessons
// inside appStore itself.
assert.match(
  agentConversationRunner,
  /await this\.refreshCompletedHostTurn\(active\)[\s\S]*?so generated lessons appear/,
  'conversation completion should surface lessons generated by the generate_lesson tool by refreshing the workspace catalog'
)

assert.match(
  appStore,
  /onGeneratedLessons:\s*\(lessons\)\s*=>\s*\{\s*const effects = effectsForAgentGeneratedLessons\(\{/,
  'store must react to agent-generated lessons through the shared lesson-effect path'
)

assert.match(
  appStore,
  /const lessonMessages = options\?\.messages \?\? \(\s*activeTeachingConversationSummary\(/,
  'direct lesson generation should derive context from an active teaching conversation'
)

assert.match(
  appStore,
  /messages: lessonMessages/,
  'lesson generation should send active teaching conversation history to the generator'
)

assert.doesNotMatch(
  app,
  /agentChat\(isTeachingMode \? inputValue : undefined\)/,
  'teaching-mode submit must not indiscriminately route every concept learning request through temporary agent chat'
)

assert.doesNotMatch(
  appStore,
  /messages:\s*get\(\)\.agentTurns/,
  'lesson generation must not inject raw or temporary chat history into course design'
)

// Root sidebar conversations are temporary-only. Course/durable conversations
// live under the course tree, not the flat root conversation list.
assert.match(
  app,
  /<TeachingWorkspaceNavigator[\s\S]*temporaryConversations=\{appState\.temporaryConversations\}/,
  'root conversation sidebar should only show temporary conversations'
)

assert.match(
  navigator,
  /temporaryConversations:\s*visibleTemporaryConversations[\s\S]*visibleTemporaryConversations\.map\(\(conversation\)\s*=>\s*<ConversationListRow/,
  'navigator flat conversation list must render only the temporary conversation projection'
)

assert.doesNotMatch(
  navigator,
  /temporaryConversations=\{[^}]*workspace\.conversations/,
  'navigator must not project durable workspace conversations into the temporary list'
)

assert.doesNotMatch(
  app,
  /appendClarificationTurns/,
  'renderer must not append local TeachingClarificationResult.assistantMessage as an assistant reply'
)

assert.doesNotMatch(
  appStore,
  /overviewDialogMode:\s*'chat',\s*\n\s*lessonReaderOpen:\s*false,\s*\n\s*(?:selectedCoursePreviewFile:\s*null,\s*\n\s*)?appState:\s*[^,\n]+,\s*\n\s*agentTurns:/,
  'generation clarification fallback must not force the overview switch to chat mode'
)

console.log('overview teaching mode routing ok')
