...
const [app, workbench, leaderboard, roomSwitcher, pomodoro, tasks, viewModel, shellCss, cardsCss, scheduleCss, immersiveCss, timerCss, leaderboardPillCss, disclosureReveal] = await Promise.all([
  ...
  readFile('src/renderer/src/views/workbench/useWorkbenchDisclosureReveal.ts', 'utf8')
])
...
assert.match(
  workbench,
  /<WorkbenchLeaderboard[\s\S]*members=\{leaderboardMembers\}[\s\S]*presenceStatus=\{presence\.status\}[\s\S]*spaceCode=\{snapshot\.spaceCode\}[\s\S]*onEnterRandomSpace=\{handleEnterRandomSpace\}[\s\S]*onJoinSpace=\{handleJoinExistingSpace\}[\s\S]*\/>/,
...
assert.match(
  tasks,
  /workbench-task-toggle-label[\s\S]*任务清单[\s\S]*workbench-task-toggle-meta[\s\S]*\{openTasks\} 待办/,
...
assert.match(
  ...
  'disclosure cards should retain their expanded geometry while the content collapses'
)
assert.match(
  leaderboard,
...
assert.match(
  roomSwitcher,
  /workbench-leaderboard-actions[\s\S]*<form className="workbench-room-join"[\s\S]*<CornerDownLeft[\s\S]*workbench-room-random[\s\S]*aria-label="随机分配新自习室"/,
  'expanded leaderboard footer should render the enter-key join action before the icon-only random action'
)

assert.match(cardsCss, /\.workbench-tools \.workbench-leaderboard-toggle \{/, 'workbench leaderboard button should have dedicated styling')
assert.match(
  leaderboardPillCss,
...
assert.match(
  leaderboardPillCss,
  /\.office-workbench-stage > \.workbench-leaderboard\.is-open,[\s\S]*\.workbench-leaderboard\.is-closing \{[\s\S]*border-radius: 24px;/,
  'expanded and closing leaderboard should preserve the compact pill width and only soften the outer corners'
)
assert.doesNotMatch(
  leaderboardPillCss,
...
assert.match(
  scheduleCss,
  /\.workbench-disclosure-reveal \{[\s\S]*height 300ms cubic-bezier\(0\.65, 0, 0\.35, 1\)[\s\S]*opacity 300ms cubic-bezier\(0\.65, 0, 0\.35, 1\)/,
  'leaderboard should use the same symmetric 300ms height and opacity motion for opening and closing'
)
assert.match(
  scheduleCss,
  /\.workbench-disclosure-card\.is-open \.workbench-disclosure-reveal \{[\s\S]*height 300ms cubic-bezier\(0\.65, 0, 0\.35, 1\)[\s\S]*opacity 300ms cubic-bezier\(0\.65, 0, 0\.35, 1\)/,
...
assert.match(
  disclosureReveal,
  /WORKBENCH_DISCLOSURE_REVEAL_DURATION_MS = 300[\s\S]*?window\.setTimeout\([\s\S]*?WORKBENCH_DISCLOSURE_REVEAL_DURATION_MS\)/,
...
assert.match(
  scheduleCss,
  /\.office-workbench-stage > \.workbench-leaderboard\.is-open \{[^}]*width: min\(292px,[^}]*max-width: min\(292px,/,
...
assert.match(
  scheduleCss,
  /\.workbench-disclosure-reveal \{[\s\S]*height: 0;[\s\S]*overflow: hidden;[\s\S]*height 300ms/,
  'collapsed leaderboard content should be vertically clipped with a smooth height transition'
)
assert.match(
  disclosureReveal,
  /const \[revealHeight, setRevealHeight\] = useState\(0\)[\s\S]*revealRef[\s\S]*scrollHeight/,
  'disclosure cards should measure their detail content and animate an explicit height in both directions'
)
assert.match(
  leaderboard,
  /style=\{\{ height: `\$\{revealHeight\}px` \}\}/,
  'leaderboard should bind the measured height to its reveal container'
)
...
assert.match(
  scheduleCss,
  /\.office-workbench-stage \.workbench-tools \{[\s\S]*height: var\(--workbench-tools-layout-height\);[\s\S]*max-height: var\(--workbench-tools-layout-height\);[\s\S]*padding-bottom: 0;[\s\S]*overflow-x: visible;[\s\S]*overflow-y: auto;/,
...
assert.match(cardsCss, /\.workbench-leaderboard-panel \{/, 'expanded workbench leaderboard should have dedicated styling')
assert.match(
  cardsCss,
