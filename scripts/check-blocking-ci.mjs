...
const gatewayContext = await readFile(resolve(root, 'src/main/teaching-ipc-gateway-context.ts'), 'utf8')
const workspaceCommands = await readFile(resolve(root, 'src/main/teaching-workspace-ipc-commands.ts'), 'utf8')
...
assert.match(gateway, /\.\.\.registration/, 'gateway must compose the registration (which carries the coordinator host)')
assert.match(gatewayContext, /turnCoordinatorHost\?: TeachingTurnCoordinatorHost/, 'gateway context must accept coordinator host')
assert.match(workspaceCommands, /if \(context\.turnCoordinatorHost\) return context\.turnCoordinatorHost\.commitLearningOutcome\(request\)/, 'workspace IPC group must route commits through host when present')
